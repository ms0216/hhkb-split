    ...
    **autoroute.py も SES 取り込みのあとに呼ぶ**（取り込みが既存の配線を
    ...
    for lane_i, (name, pads) in enumerate(sorted(cols.items())):
        ...
        for a, b_ in zip(pads, pads[1:]):
            ...
            #
            # （open-gaps #46 は 2026-08-17 に dtsi 側も修正済み。
            # 幅広キーの列ずれは Alt など配置由来の残りだけなので、
            # この短縮策は引き続き有効。）
            #
...
J_DB_DY = -3.7    # 左の値。右は -2.5（配置表の注記）。 口を奥向きに回したとき、パッドが回す前と同じ y に来る量（KiCad の Y は下向き。奥＝負）
...
PLACE = {
    ...
    "left": {
        ...
        # マージンを動かすとここも動く。
        # **向き 0°＝口が奥向き**（2026-08-28・open-gaps #19 再開）。以前は
        # Flip の副作用の 180°（口が手前）をそのまま使っていて、誰も向きを
        # 決めていなかった。ケーブルは J_DB を奥へ出てそのまま子基板へ入る。
        # dy はパッドが回す前と同じ y（帯の中の同じ位置）に来る値。
        "J_DB": (1, J_DB_X, 0, J_DB_DY),
    ...
    "right": {
        ...
        # 向きは左と同じ（口が奥向き）。**右は左と逆に 1.235mm 手前**（中心 y=88.46）。
        # 奥へ寄せると SW10 の位置決め穴がコートヤードに入り、帯の中心だと
        # SW10 のソケットの保守的占有範囲と 1.63mm 重なって FFC の出口の曲げが
        # きつくなる（2026-08-30・open-gaps #19）。ここでソケットまで 3.9mm。
        # J_DB 周りの配線は利用者が引く（matrix_only の現物が優先）。
        "J_DB": (1, J_DB_X, 0, 1.235),
        # **C_BULK はここに無い。子基板へ移した**（open-gaps #41）。
