...
PLACE = {
    ...
    "left": {
        ...
        # 詰めた）。63.0 のみ未配線 0（3 回実行して決定的）。
        # 子基板コネクタの中心 X は +59.5mm（gen_case.daughterboard_x_center）
        # で、63.0 はそこから 3.5mm——ほぼ真横に来る。
        "J_DB": (1, 63.0),
    ...
    "right": {
        ...
        # **J_DB は帯 1・x=-78.0。**左と同じ理由（帯 0 に置くと未配線が
        # 3〜27 本残る。帯 1 なら 1 本まで減る。実測で -78 が最良、
        # 周辺 ±20mm を刻んで確認した）。右基板は列が 9 本あり
        # 590 系が 2 個あるぶん帯 0 の混雑が左よりきつく、**0 まで
        # 詰め切れなかった。**残る 1 本（COL8-SW31、基板最遠端の列）は
        # gnd_fanout の格子や周辺部品の再配置では解消しなかった
        # （2026-08-13 に系統的に確認。故意に固定してみて毎回同じ
        # ネットが落ちることを確かめた——揺れではなく構造的な残り）。
        # 子基板コネクタの中心 X は -76.2mm、-78.0 はそこから 1.8mm。
        "J_DB": (1, -78.0),
