...
def _courtyard_bbox(blk):
    ...
    # しか見ておらず「コートヤードが無い」と誤判定していた（#49 で発覚）。
    for m in re.finditer(
...
# 「この値なら十分」という主張ではない。
#
# **2026-08-25 に 6.0 → 6.5。**#51 の J_DB 引き直し後、右の C_U2 は
# 干渉限界まで寄せて 6.21mm（利用者の実配置）。6.0 は上と同じく
# 丸めた経験則なので、実配置に 0.3 の余裕を足した 6.5 へ。番人の目的
# （17mm の再発防止）は保たれる（現状: 左 C_U1 4.97 / 右 C_U1 2.76 /
# 右 C_U2 6.21）。
MAX_DECOUPLE_MM = 6.5
