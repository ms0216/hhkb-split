...
#
# ⚠️ **アクチュエータの突出量は確定していない**（2026-08-12 の調査）。
# 設計は「長さ 4.00mm → 壁 2.4mm を貫いて 1.6mm 出る」を前提にしているが、
# **4.00mm が本体面からの突出量である保証が無い。**
#   4.0 説: 8.4（つまみ込みの奥行・2 販社）− 4.4（本体奥行）= 4.0
#   2.8 説: 同族の垂直版 OS102011MS2QN1 は全高 7.5 − 枠高 4.7 = 2.8。
#           つまり C&K の言う "Actuator Length 4.00" は突出量ではない
# **要求は 3.4mm 以上**（壁 2.4 ＋ 外へ 1.0）なので、この差が合否を分ける。
# → **¥121 で 1 個買ってノギスで測るのが唯一の決着手段。**
#
# ⚠️ **壁厚 2.4mm を貫く要求は、部品選択を極端に狭める。**市場の超小型
# スライドの突出量は **1.4〜3.0mm が普通**で、3.4mm 以上は稀（同調査で
# 8 品番を当たった結果）。**スロットの周りだけ壁を薄くする**（外側に
# 1.0mm の窪みを作る）ほうが、部品側の自由度がはるかに大きい。
# 耳（取付フランジ）付きは**最小でも 19mm 級**しか無く、枠 10.6mm には
# 入らないので、**ポケット＋抜け止めの方針自体は正しい**と確認できた。
