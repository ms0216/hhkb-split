...
#
# 候補は C&K OS 系の超小型スライドスイッチ（例 OS102011MA1QN1）。
# データシート実測で本体 8.6 x 4.3 x 約4mm。**買う製品が変わったら
# ここの 3 つを差し替えるだけで、収まるかどうかを検査が判定する。**
#
# 向き: 幅 = 背面パネルに沿った左右、奥行 = パネルの裏、高さ = 上下。
SW_PWR_W = 4.3           # [暫定] 電源スイッチ本体の幅（左右）
SW_PWR_D = 8.6           # [暫定] 電源スイッチ本体の奥行（パネル裏）
SW_PWR_H = 4.0           # [暫定] 電源スイッチ本体の高さ（上下）
