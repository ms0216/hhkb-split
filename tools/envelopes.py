...
BATT_BOX_L = 109.0       # [確定] 電池ボックスの長辺。**利用者がノギスで実測 109.0**
                         # （2026-09-06。図面 109±1.0 のちょうど中央）
    ...
    **ソケットは socket_envelope に分けた**（open-gaps #29）。ソケットは
    ...
    with BuildPart() as env:
        with BuildSketch():
            ...
            # プレートへ締めるバカ穴（open-gaps #36・2026-08-12）。
            # **基板の側にも空けないと、柱とネジが基板に食い込む。**
            from interface import pcb_mount_positions
            with Locations(*pcb_mount_positions(half)):
                Circle(M2_CLEAR_D / 2, mode=Mode.SUBTRACT)
...
#
# ★ **2026-08-12・利用者の決定: プロトタイプ期はソケットを挟む。**
# 「何度も着け外しするから」「0.72mm は本件に限っては許容する」。
# **承知のうえの逸脱**なので、open-gaps #27 に承知の記録を書き、
# `test_the_antenna_keeps_its_distance_from_the_main_board` がその記録が
# 無ければ落ちるようにしてある（export_fab の門と同じ形）。
#
# **基板は変わらない。**変わるのはケース（USB の穴の高さ）だけなので、
# 脱プロトタイプでは `DB_SOCKET_SEAT = 0` に戻してケースを刷り直す。
DB_SOCKET_SEAT = 8.5     # [暫定] 普通のメスの座高。**プロトタイプ限定**。買って測る
...
#
# **SS12D01G4-G（秋月 112723）で確定**（利用者・2026-08-12）。
# SWITRONIC のデータシート V2.3 と秋月の商品ページ、**2 つの出所が
# 一致している**値を使う。
#
#   本体（壁に当たる面） 8.8 × 3.9 ／ 本体の高さ（座面から） 3.5
#   ツマミ  本体上面から **4.0** 突出・幅 1.5・ストローク 2.0
#   端子    3 本・2.5 ピッチ・0.5 × 0.3・本体下面から **5.0**
#   商品ページの「長さ 7.5mm」＝ 3.5 + 4.0 で図面と合う
#
# **図面と商品ページの値であって、現物を測った値ではない。**届いたら
# ノギスで確かめて入れ直すこと。
#
# ⚠️ **以前の前提（C&K OS102011MA1QN1・右アングル）とは種類が違う。**
# こちらは基板に垂直に出るストレート型。**それでも設計は変わらない**——
# この案件ではスイッチを基板に載せず、壁のポケットへ落とし込んで
# リード線で繋ぐので、**基板とパネルの位置関係という問題が発生しない。**
# 本体の上面を壁の内面に当て、ツマミがスロットから出て、端子は内側を向く。
#
# **縦向きに取り付ける。**本体 8.8 × 3.9 のうち長いほうを上下にする。
# 横向きにすると受けの箱が左半分の枠 10.6mm に入らない（旧部品で実測。
# 新部品は 3.9mm と細いので、縦向きならさらに余裕がある）。
#
...
SW_PWR_W = 3.9           # [暫定] 本体の幅（左右）＝ 本体の短辺
SW_PWR_BODY_D = 3.5      # [暫定] 本体だけの奥行（壁の内面 → 本体の背面）
SW_PWR_LEAD_D = 5.0      # [暫定] 端子が本体の背面から出る長さ
SW_PWR_WIRE = 2.5        # [暫定] 端子の先にリード線を回すぶんの余裕
SW_PWR_D = SW_PWR_BODY_D + SW_PWR_LEAD_D + SW_PWR_WIRE   # 11.0
SW_PWR_H = 8.8           # [暫定] 本体の高さ（上下）＝ 本体の長辺
SW_PWR_KNOB = 4.0        # [暫定] ツマミの突出量（本体の上面から）
SW_PWR_TRAVEL = 2.0      # [暫定] ツマミのストローク
SW_PWR_PIN_W = 0.5       # [暫定] 端子の幅（0.5 × 0.3。3 本が 2.5 ピッチで並ぶ）
SW_PWR_KNOB_W = 1.5      # [暫定] ツマミの断面（1.5 角）
...
#
# **実部品で確定**（2026-08-16）。uxcell 12 芯 0.5mm ピッチ 80mm **A タイプ**
# （同面。両端の補強板＝接点が同じ面）20 本入り。
#
# **A タイプで正しい根拠は 3 つ揃っている：**
#   1. 姿勢 … 本体基板は TILT_DEG=7.3° で傾き、子基板は床に水平。
#      J_DB・J_MAIN とも B.Cu（下向き）なので、ケーブルは**ひねらずに**
#      素直に渡る。ひねりが要るなら B タイプだった
#   2. ネット … J_DB と J_MAIN の pad 1..12 のネット並びが完全に一致
#      （鏡像ではない）。ひねらないから並びも反転しない
#   3. 実物 … 商品写真で両端の補強板が同じ面に貼られていることを確認
#
# 長さは 50/100 が主流だが 80mm が実在した。必要 54.0mm
# （直線 29.0 ＋ FFC_SLACK 25.0）に対し余り 26mm。100mm だと 46mm 余り、
# コブの中で折り返す量が増える（アンテナ #23 の近くで銅箔を余らせたくない）。
# **50mm は 4mm 足りない。**端子に補強板があるので継げず、買い直しになる。
FFC_LENGTH = 80.0        # [確定] uxcell 12 芯 0.5mm A タイプ 80mm
...
SCREW_L_MAIN = 14.0      # [暫定] **裏から**（2026-09-06）: 底の座ぐり → 床 → ボス柱 →
                         # プレート → 上シェルのバーのインサート（噛み合い 4.0）
SCREW_L_DB = 5.0         # [暫定] 子基板 → 子基板ボスのインサートまで
SCREW_L_REAR3 = 8.0      # 奥面 3 本目（奥壁 2.4 ＋ 隙間 0.2 を貫いて上シェルの
                         # ボスのインサート 4.0 に噛む。5 では噛み合い 2.4）
# 本体基板 → プレート裏の柱（open-gaps #36・2026-08-12）。
# **下から入れる**ので、板厚 1.6 ＋ 柱 3.5 = 5.1mm 噛む長さが要る。
# 5.0 なら板を貫いて柱へ 3.4mm。タッピング（下穴 φ1.6）なので十分。
SCREW_L_PCB = 5.0        # [暫定] 本体基板 → プレート裏の柱
