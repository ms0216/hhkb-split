...
def build():
    ...
    # パスコン（裏面・XIAO の下）。
    #
    # ⚠️ **2026-08-16 まで、ここにバルク（C_BULK 100µF）も並んでいた。**
    # open-gaps #44 で削除した（実測で不要と判明）。**残るのは C_DB のみ。**
    #
    # **電源ピンに近い側がパスコン（C_DB）。**2026-08-14 に入れ替えた
    # （利用者の指摘「バルクがパスコンを兼ねられないのか」を検討した
    # 副産物）。**兼ねられない。**自己共振を超えるとインダクタになるので、
    # 帯域が違う:
    #
    #     C_DB   0.1µF/0805  自己共振 約 19MHz   ns 級の縁を担当
    #     （旧 C_BULK 100µF/1206  約 0.5MHz  µs 級のバースト担当）
    #
    # **兼ねられないという結論は変わらない。**変わったのは「µs 級を担当する
    # 相手が実在しなかった」という事実のほうで、**バルク側が消えた。**
    # パスコンが要る理由（ns 級・ループ長）は無傷。
    #
    # **ループ長が効くので、電源ピンに近い枠は C_DB に与える。**
    ...
    # ⚠️ **C_BULK（バルク 100µF）は 2026-08-16 に削除した**（open-gaps #44）。
    #
    # ここには (3.00, 10.97)・+90° で 1206 を置いていた。**実測で
    # 「守る相手が存在しない」と分かったので消した**（判断の全体は
    ...
    #
    # 消えた副産物として、**この 1206 が占めていた 2.1×3.5mm が空く**。
    # 電源部の置き場所に余裕ができた。
    #
    # ⚠️ **C_DB（パスコン 0.1µF）は上に残っている。**役割が別で、
    # IC の電源ピン直近で ns 級を返す担当。**混同しないこと。**
