...
# ⚠️ **CAP_FP は消した**（2026-08-16）。子基板にコンデンサが 1 つも
# 無くなったため（C_BULK #44 / C_DB は decisions/
# 2026-08-16-cdb-has-no-local-load.md）。**主基板の C_U1/C_U2 は健在**で、
# そちらのフットプリントは gen_pcb 側が持っている。
...
POWER_ROT = {
    ...
    #   D_PWR  180°  … 横置き。VBATT_SW が左、V3V3 が右（C_DB 側）
    #
...
#
#   奥の分圧   y … R_LO(-5.35, 13.7675) → R_HI(-2.01, 14.68)。R_HI は横置き
#   スイッチ           … SW_PWR(-0.68, 12.20)。一列から外して手前へ
#   整流               … D_PWR(1.51, 9.55) 横置き
#
# ⚠️ **2026-08-16 に利用者が KiCad で並べ直した**（C_BULK 削除で空いた
# 場所へ電源部を寄せた。open-gaps #44）。R_LO/R_HI/SW_PWR/D_PWR の 4 つと、
# `SILK_REF` のシルクが動いている。
# D_PWR は (3.50, 7.50) → (1.51, 9.55) と、旧 C_BULK(3.00, 10.97) の
# 跡地の側へ上がった。出所は `pcb/hhkb_split_daughterboard.kicad_pcb`。
#
# ⚠️ **同じ日に C_DB(5.40, 10.50) も消えた**（この並び替えの後）。
# **電源部はそのままにしてある**——上の 4 つは KiCad で釣り合いを取った
# 値で、動かすと `SILK_REF` の 0.15mm が崩れる。空いた場所は空いたまま。
POWER_PLACE = {                 # 参照名 → (x, y)
    "R_LO":     (-5.35, 13.7675),  # 分圧の下（SENSE → GND）
    "R_HI":     (-2.01, 14.68),    # 分圧の上（VBATT_SW → SENSE）。横置き
    "SW_PWR":   (-0.68, 12.20),    # 電池から来る線を受けるランド
    "D_PWR":    ( 1.51,  9.55),    # VBATT_SW → V3V3
    # 電池の −（GND）。ベタで受ける。
    #
    # ⚠️ **2026-08-17 に利用者が動かした（前は (-0.50, 3.50)）。理由は
    # 「XIAO の GND パッドに近いほうがいい」。**電池からの戻り電流が
    # XIAO の GND へ帰る距離で、**11.85mm → 3.54mm**（実測。XIAO GND は
    # (157.62, 87.87)）。**戻り経路の話なので、ベタで繋がっているから
    # どこでもよい、ではない**（decoupling-cap-is-loop-length-not-distance
    # と同じ型）。**XIAO の GND から遠ざける向きに動かさないこと。**
    "BT1":      ( 4.375, 13.55),
}
...
def build():
    ...
    # ⚠️ **子基板にコンデンサは 1 つも無い**（2026-08-16 に 2 つとも削除）。
    #
    # ここには裏面・XIAO の下に 2 つ並べていた:
    #   C_DB   0.1µF/0805  (5.40, 10.50) +90°  ループ長 4.49mm
    #   C_BULK 100µF/1206  (3.00, 10.97) +90°
    #
    # ### C_BULK（open-gaps #44）— 守る相手が実在しなかった
    #
    #   BLE の最大瞬間風速  10.22mA（広告＋スキャンの重なり込み・実測）
    #   降下 = 10.22mA × 9.2Ω = 94mV   ← **容量ゼロでもこれが上限**
    #   余裕 300mV に対して 31%（安全率 3.2 倍）
    #   µs 級の尖り  1ns 分解能・60 万点を全数走査して **0 個**
    #
    #   docs/hardware/decisions/2026-08-16-remove-bulk-cap.md
    #
    # ### C_DB — **負荷の直近にいないパスコンだった**
    #
    # ⚠️ **「相手がいない」のではない。「取り分が誤差」。**
    # 3V3 の負荷は 2 つとも**この板の外**にいて、どちらも自分の直近に
    # 別のパスコンを持っていた:
    #
    #   nRF52840 → **XIAO の内部**。内蔵の 2.2µF ×5 が桁違いに近い
    #              （Seeed の KiCad ソースを機械で数えた全数）
    #   595 ×2   → **主基板**。直近の C_U1/C_U2 が **58:1** で取る
    #
    # SGM2040（XIAO 内蔵 LDO）の安定条件 C_OUT=0.1〜10µF も、
    # 内部の公称 11µF が満たしている。**上限側は既に超えており、
    # 外から足す方向の議論はそもそも成立しない。**
    #
    #   docs/hardware/decisions/2026-08-16-cdb-has-no-local-load.md
    #
    # **戻す前に上の 2 文書を読むこと。**特に「XIAO にパスコンが無い」は
    # 誤り——内蔵している。ここ（板の隅）に足しても効かない。
    # 後から 3V3 で動く IC を子基板へ足すなら、**その IC 自身の隣**に置く
    # （`test_the_daughterboard_has_no_part_that_needs_local_decoupling`）。
    #
    # 空いた面積は 0805 + 1206 ぶん。電源部の置き場所に余裕ができた。
...
SILK_REF = {                  # 参照名 → (dx, dy, 文字角度)
    ...
    # ⚠️ **C_DB の行は 2026-08-16 に消した**（部品ごと削除。
    # decisions/2026-08-16-cdb-has-no-local-load.md）。値は (1.710, -1.380,
    # 270.0) だった。**他の行は動かしていない**ので、上の「0.15mm
    # ぎりぎり」の釣り合いはそのまま。
    "D_PWR":    ( 0.010, -1.730,   0.0),
    "H1":       ( 0.000, -2.300,   0.0),
    "H2":       ( 0.000, -2.300,   0.0),
    "J_MAIN":   ( 0.000,  3.906,   0.0),
    "R_HI":     ( 0.810, -2.230, 270.0),
    # ⚠️ **ここだけ利用者の値ではない**（利用者は (1.4475, -1.390)）。
    # 利用者のファイルは**ゾーンを塗り直していない**ので DRC が別の
    # 21 件（clearance / copper_edge_clearance / solder_mask_bridge …）で
    # 埋まっていて、この 1 件が隠れていた: ラベルが**自分の外形線**まで
    # 0.0823mm しかない（要 0.15mm）。
    #
    # **左右には逃がせない。**R_LO のラベルは x 145.309..146.771 で、
    # 自分の外形線が x=145.625 まで、R_HI のラベルが x=146.449 から。
    # どちらへ振ってもどちらかに当たる（dx/dy を 20 通り試して全滅）。
    # 空いているのは 2 つの部品より手前だけなので、**手前へ**出した。
    #
    # ⚠️ **R_LO は裏面で Flip されているので dy の符号が反転する。**
    # 手前へ出すつもりで -2.30 と書いたら、逆に板の端へ寄って
    # silk_edge_clearance と silk_over_copper が出た（0.0099mm）。
    # 正しくは **+**。2.30/2.60 はまだパッドに乗り、**2.90 から 0 件**。
    "R_LO":     ( 2.900,  0.000, 270.0),
    # **2026-08-17 に利用者が動かした**（前は (-3.390, 2.130, 180.0)）。
    # BT1_- が奥へ抜けたので、上の「0.01mm の釣り合い」は組み替わっている。
    "SW_PWR_1": ( 1.320, -4.550, 270.0),
