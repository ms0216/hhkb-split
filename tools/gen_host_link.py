...
# ======================= ① 実測と当てはめ =======================
txt(X0, 96, "① latency 0 で間隔を振る（点が足りない）",
    "#111", 13.5, "start")
...
txt(X0 + 10, py(0.055), f"式: {C_FIT:.3f} ＋ {K_HOST:.3f} ÷ 間隔[ms]", "#111", 11, "start")
txt(X0 + 10, py(0.028), "⚠️ 45 と 60 が同値なので、実質 2 点でしか決まっていない。",
    RED, 10.5, "start")
txt(X0 + 10, py(0.005), "「1/間隔」の形も、分割リンク（8 点）からの類推で、ここでは未検証。",
    RED, 10.5, "start")
...
txt(44, TY2 + 216, "⚠️ 未解決 ②: ①の当てはめは 2 点でしか決まっていない。"
                   "60ms は採られた値をログで確かめておらず、測定中に行き来もしていた。",
    RED, 12, "start")
txt(44, TY2 + 236, "→ 行き来を先に片づけてから、30 / 90 / 120ms を足して引き直す。",
    "#666", 11.5, "start", bold=False)
