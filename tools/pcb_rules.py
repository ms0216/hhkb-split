"""基板の設計規則の数値。**pcbnew を要らない形で持つ。**

なぜ別ファイルか
----------------
`gen_pcb.py` は `import pcbnew` するので、**KiCad の Python でしか
読めない。**一方この案件の検査は母艦の venv（pcbnew 無し）で動く。
数値を gen_pcb に置いたままだと、検査から参照した瞬間に
`ModuleNotFoundError: No module named 'pcbnew'` で落ちる（実際に落ちた）。

同じ理由で `tools/part_kinds.py` を作ったことがある（あちらは build123d が
Blender の Python に無い問題）。**依存を持たない側へ数値を出す**のが
この案件での定石。

⚠️ **層の指定（F_Cu など）はここに置けない。**pcbnew の定数なので
gen_pcb 側に残す。
"""

# ベタの小片を捨てる面積のしきい値（mm²）。
#
# **ここで捨てるのは「ビアが物理的に入らない小片」だけ。**
# ビアの外径 0.6mm にクリアランス 0.2mm を足すと直径 1.0mm、
# 面積にして約 0.8mm²。それを下回る区画にはどうやってもビアを立てられない
# ので、繋ぎようがない。**それより大きい離島は、消さずにビアで繋ぐ**
# （gnd_fanout.stitch_islands）。
MIN_ISLAND_MM2 = 1.0

# --------------------------------------------------------------------------
# 配線の寸法
#
# **配線そのものはここではやらない。**Freerouting に委ねる
# （tools/autoroute.py）。ここで持つのは、自動配線器へ渡す値だけ。
#
# 手書きのルータは削除した。衝突判定を持たないので任意のネット対を
# 短絡させ、それは経路の調整では 0 にならなかった。経緯は
# docs/superpowers/specs/2026-08-08-pcb-autoroute-design.md。
#
# **この 3 つは _apply_jlcpcb_rules がネットクラスに書き込む。**
# 自動配線器はネットクラスしか見ないので、ここが唯一の出どころ。
# --------------------------------------------------------------------------
TRACK_W = 0.2           # JLCPCB の最小 0.127mm に対して余裕を見た値
VIA_D, VIA_DRILL = 0.6, 0.3

# **電源のネットは太くする**（2026-08-12・指摘 8）。
#
# 経験則で 1A あたり 1mm。この設計が流す電流はまるで足りていない——
# BLE の送信で 15mA、レール全体でも数十 mA なので、0.2mm でも桁で足りる。
#
# **それでも太くするのは、切り分けのため。**基板を評価していて挙動が
# おかしいとき、「電源ラインが細いせいでは」という初歩的な疑いに時間を
# 使いたくない。
#
# **なぜ 0.5mm ではなく 0.3mm か（実測で決めた上限）**
#
# 最初 0.5mm にしたら、**両基板とも V3V3 が J_DB に届かず未配線になった。**
# 原因を測ったら物理的に不可能だった:
#
#   J_DB（Hirose FH12-12S-0.5SH・0.5mm ピッチ）のパッドは
#   **幅 0.30mm、隣との隙間 0.20mm。**
#   0.5mm の線は 0.30mm のパッドに載らない。
#
# V3V3 はこのコネクタを通って子基板へ行くので、**0.30mm がこの設計の
# 上限**。利用者も「基板面積の制約があるので絶対に全部太くしろとは
# 言わない」と条件を付けている。ここはその条件に当たる。
#
# 0.3mm でも 1A/1mm の目安に対して 0.3A 相当。実際に流す 15〜30mA の
# 10〜20 倍あり、「電源が細いせいでは」という疑いを消す目的は達している。
#
# **ネットごとに幅を変えられる**（2026-08-12 に確認）。
#
# 当初「Freerouting はネットクラスごとに 1 つの幅しか受け取れない」と
# 書いたが、**確認せずに断定した誤りだった。**実際に KiCad が出す DSN を
# 読むと、クラスが 2 つ並んで書かれている:
#
#     (class kicad_default COL0 COL1 ... (rule (width 200) ...))
#     (class Power GND V3V3 VBATT_RAW VBATT_SW (rule (width 300) ...))
#
# **できないのは「1 本のネットの中で場所により幅を変える」ほう**
# （開けた所は太く、細ピッチのパッド際だけ絞る＝ネックダウン）。
# そこだけは配線後の後処理で太らせる（gnd_fanout.widen）。
#
# そこで電源を 2 つのクラスに割る。**FFC に届く必要があるのは V3V3 だけ**
# なので、他のネットを 0.30mm に引きずられる理由が無い。
# ⚠️ **0.3 では FFC のパッド列から脱出できない**（2026-08-14 に実測）。
#
# 以前ここは 0.3 で、理由を「FFC の 0.30mm パッドに届く必要がある」と
# 書いていた。**パッドの幅と、パッドの隙間を取り違えている。**
# 幅 0.30 の線はパッドには載るが、**隣のパッドの間を通れない。**
#
#     FFC は 0.5mm ピッチ・パッド幅 0.30 → 隣のパッド端まで
#       幅 0.30 で出る … 0.200mm（規則ちょうど。丸めで即割る）
#       幅 0.20 で出る … 0.250mm（通る）
#
# **右基板で V3V3 だけが J_DB から出られなかった**（未配線 2 本）。
# 自動配線器はビアだけ (72.75, 84.00) に置いて両端とも繋がず放棄した。
# 同じ列の VBATT_SENSE・SPI_MOSI・SPI_SCK・CS は全部 0.20mm なので
# 同じ形で脱出できていた。**V3V3 だけが自分の太さで詰まっていた。**
#
# 電流は BLE のパルスで 15mA。0.20mm でも桁が違うほど余る
# （electrical-design 1-5）。バルクは子基板側にあるので、この線が
# 供給するのは平均電流だけ。
POWER_TRACK_W = 0.2
# クラス名 → (線幅 mm, そのクラスに入れるネット)
#
#   PowerFFC   V3V3 だけ。**FFC のパッド列を抜けられる幅**にする。
#              太さより「通れること」が先（上の注記）。
#   PowerWide  それ以外の電源。繋ぐ先は 2mm のランド・SOD-123・0805 で、
#              細ピッチのパッドが無い。最初から太く引ける
POWER_CLASSES = {
    "PowerFFC":  (POWER_TRACK_W, ("V3V3",)),
    # VBATT_RAW は 2026-08-14 に廃止（電池の + はスイッチへ直結・#41）。
    "PowerWide": (0.6, ("GND", "VBATT_SW")),
}
POWER_NETS = tuple(n for _w, nets in POWER_CLASSES.values() for n in nets)

# --------------------------------------------------------------------------
# JLCPCB の製造能力を、基板の設計規則として書き込む。
#
# **これが無い間、DRC は KiCad の既定値で通していただけだった。**
# 「違反 0 件」は「JLCPCB で製造できる」を意味していない。規則を入れて
# 初めて、線幅・ビア・アニュラリング・外形までの距離が能力の内側に
# あることを機械が確かめられる。
#
# 値は JLCPCB の Capabilities（2層/4層・1oz・標準工程）から。
# 追加費用のかかる高精度オプションは使わない前提で、標準値を採る。
# --------------------------------------------------------------------------
JLC = {
    "track_min": 0.127,       # 最小線幅 5mil
    "clearance_min": 0.127,   # 最小クリアランス 5mil
    "via_dia_min": 0.45,      # 最小ビア外径
    "via_drill_min": 0.20,    # 最小ビアドリル
    "hole_min": 0.20,         # 最小 PTH ドリル
    # ⚠️ **0.50 は根拠なく厳しかった**（2026-08-23）。公式は **0.45**
    # （Pad Hole-to-Hole Spacing）。0.05mm の上乗せで、Kailh MX
    # ホットスワップの内部（実測 0.4398mm）が実際より深刻に見えていた。
    # **根拠のない厳しさは、本当の問題を隠す。**open-gaps #50。
    "hole_to_hole": 0.45,     # 穴どうしの最小距離（JLCPCB 公式）
    "edge_clearance": 0.30,   # 銅から基板外形までの最小距離
    "silk_width": 0.15,       # シルクの最小線幅
    "annular_ring": 0.13,     # 最小アニュラリング。KiCad 既定は 0.1 で足りない
}


# **パスコンは IC の直近に置く。座標を手で書かない。**
#
# 手で書いていた結果、**C_U1 が U1 の VCC ピンから 17.0mm 離れていた**
# （2026-08-12 に実測。指摘 6）。この距離ではパスコンとして働かない。
# しかも当時は C_RAIL（さらに前は C_MCU）のほうが U1 に近いという、
# 逆転した並びだった。**その C_RAIL は 2026-08-14 に削除した**
# （置き場所の根拠が持たず C_U1 の重複だった。open-gaps #39・circuit.py）。
#
# パスコンで効くのは配置上の直線距離ではなく、
# **IC の電源ピン → パスコン → GND → 地板 → IC の GND ピン**と一周する
# 経路の配線長（ループのインダクタンス）。だから
#
#   - 置く側は「相手の V3V3 パッドがある側」を**実際のパッド座標から決める**
#   - コートヤードが触れない最短の位置まで寄せる
#   - GND 側の戻りは地板で受ける（gnd_fanout が各 GND パッドの脇にビアを立てる）
#
# 通ったかどうかの判定は**配線後の実測**で行う
# （test_pcb.test_decoupling_caps_are_close_to_their_ic_in_copper）。
DECOUPLE_BESIDE = {"C_U1": "U1", "C_U2": "U2"}

# 算出位置からのずらし（mm・KiCad の Y は下向き）。**既定は 0。**
#
# ⚠️ **手で座標を書くための枠ではない。**行き先は今までどおり
# `_place_beside` が IC の V3V3 パッドから算出する。ここに書けるのは
# その答えからの差分だけなので、IC を動かせばパスコンも付いてくる。
# （座標を直接書ける仕組みにしたら、C_U1 の手書き座標が 17mm ずれた
# 事故が再発する。指摘 6。）
#
# **入れてよいのは実測で効くと分かったときだけ。**
# C_U1 は 2026-08-17。利用者が基板上で直接置いた位置で、算出位置より
# J_DB 寄り。**ただし未配線 0 との因果は取れていない**——この板は
# gen_pcb を回すたびに未配線が 0/1 で揺れる（KiCad の並び順が変わり、
# Freerouting の探索順が変わるため。同じ .kicad_pcb を繰り返し配線する
# 限りは決定的）。**「このずらしが 0 件にした」とは言えない。**
#
# ⚠️ **基板ごとに分ける。**左右は同じ参照名（C_U1）を使うので、
# 名前だけで引くと**左のために測った値が右にも黙って掛かる。**
# 実際に踏んだ（右の C_U1 が 0.92/2.28mm 動き、右だけを再生成した
# つもりの差分に紛れた）。効くと確かめた基板にだけ書く。
DECOUPLE_OFFSET = {
    # 左も利用者が置き直した（2026-08-23・扇形を引くときに C_U1 を
    # U1 の上へ寄せた。板 (201.950, 91.000) → (201.070, 86.500)）。
    "left": {"C_U1": (0.0425, -2.225)},
    # 右は利用者が置いた位置（2026-08-18・「試しに matrix の右手
    # U1, U2 パスこんの位置を考えてみた」）。算出位置からの差分。
    # IC を動かせば付いてくる。
    "right": {"C_U1": (6.117, -3.880), "C_U2": (8.320, -0.273)},
}

# **パスコンの向き（度）。**既定は `_place_beside` が決める
# （V3V3 側のパッドが IC を向くように 180 度回すだけ）。
#
# ⚠️ **利用者が IC を回したら、パスコンも回る。**2026-08-22 に U2 を
# 90 度回し、C_U1/C_U2 もそれぞれ 0 度 / -90 度にした。算出では出ない
# ので、ここに書く。書いた値は `_place_beside` の自動回転より優先する。
DECOUPLE_ANGLE = {
    "left": {},
    "right": {"C_U1": 0.0, "C_U2": -90.0},
}

# コートヤードどうしの隙間（mm）。0 にすると DRC のコートヤード重なりで落ちる。
BESIDE_GAP = 0.25


# --------------------------------------------------------------------------
# オシロ／リード線用に露出する銅（open-gaps #49・2026-08-25）。
# finalize_pcb.expose_debug_copper が使い、test_pcb が見張る。
# **ここに置くのは、検査環境（.venv）に pcbnew が無く finalize_pcb を
# import できないため。**
# --------------------------------------------------------------------------
DEBUG_NETS = {
    "left":  ["SPI_SCK", "SPI_MOSI", "CS", "V3V3"],
    "right": ["SPI_SCK", "SPI_MOSI", "CS", "U1_U2", "V3V3"],
}
DEBUG_WINDOW_L = 2.5     # 窓の長さ（配線に沿って・mm）
DEBUG_WINDOW_W = 0.5     # 窓の幅（配線 0.2 + 片側 0.15・mm）。0.8 では隣の銅との
                         # レジストの残り幅が規定を割り DRC 8 件（2026-08-25）
# 部品の本体（コートヤード）からこれだけ離れた銅しか露出しない。
# 利用者の指摘（2026-08-25）: 右の露出ビア 3 点が J_DB コネクタの真下で、
# 実質半田付けできなかった。銅の孤立だけ見て部品の体積を見ていなかった。
DEBUG_ACCESS_MM = 0.6     # こて先が入る余裕。1.5 だと SPI 系の銅は全部除外された
                          # （右は J_DB/U2 の密集域にしか無く、最遠でも 0.77mm）
# 露出点の、他ネットの銅までの下限。これ未満だと半田が隣へ流れる。
DEBUG_MIN_CLEAR_MM = 0.5
