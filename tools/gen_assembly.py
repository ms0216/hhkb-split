...
#   - 電池蓋が外から開かない（#35。**2026-08-12 に奥面へ移して解決**）
...
INSERT_PATH = {
    ...
    # 奥板: 足を床の溝から持ち上げてから奥へ（撓ませない。案 A）
    "rear_plate": [(0, 0, 1.2), (0, 8, 1.2), (0, 40, 1.2)],
    # 本体基板とソケットは真上（プレートを外した状態）。プレートも下シェルに
    # 対しては真上。**上シェルに対しては傾けて入れる**（奥端が棚とベゼルの
    # 溝に入る）ので、そちらは test_the_plate_can_be_put_into_the_shells が見る
    "plate":    [(0, 0, 40)],
    "pcb":      [(0, 0, 40)],
    "sockets":  [(0, 0, 40)],
}
...
HELD_BY = {
    ...
    "rear_plate": "M2×2 で上シェルの桟へ。足は床の溝、左右は奥壁の座ぐり（案 A・2026-09-05）",
}
