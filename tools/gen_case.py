...
#
#   庇 1.6 ＋ 舌 1.2 ＋ 逃げ 0.2 = 3.0 = WALL 2.4 ＋ この 0.6
REAR_LID_LIP_BOSS = 0.6
...
def build_rear_battery_lid(half, keys):
    ...
    **片持ちばねは不成立だった。**理由は定数の注記（腕の逃げ場が無い）。
    ...
    # 庇の帯の内面。**ケース側で内側へ REAR_LID_LIP_BOSS だけ厚くしてある**
    # ので、素の WALL では 0.6mm 足りない（舌が庇に食い込む）。
    y_wall_in = y_out - WALL - REAR_LID_LIP_BOSS
    ox0, oz0, ox1, _oz1 = rear_lid_opening(half, w)
    rx0, rz0, rx1, rz1 = rear_lid_rebate(half, w)
    cx = (rx0 + rx1) / 2
    ...
    with BuildPart() as lid:
        ...
        # （y_lid_in）と同じ深さで出すと庇に食い込む。
        # **厚みは「厚くした壁 − 庇 − 逃げ」。**2026-08-29 に庇を 0.8→1.6 に
        # したとき、素の WALL のままだと**舌が 0.8→0.6mm に痩せて、今度は
        # 舌が折れる**（庇と舌が同じ壁厚を取り合っている）。壁を内側へ
        # REAR_LID_LIP_BOSS 厚くしたので、舌は 1.2mm を保てる。
        # **板の内面から内側へ伸ばす。**帯の内面から出すと、板との間に
        # 0.10mm の隙間が空いて**舌が板から分離する**（2026-08-29 に
        # この関数の自己検査「蓋が 2 個に分かれている」が捕まえた）。
        _tongue_t = WALL + REAR_LID_LIP_BOSS - REAR_LID_LIP_T - CLEARANCE
        with BuildSketch(Plane.XZ.offset(-y_lid_in)):
