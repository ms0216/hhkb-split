...
PARTS = {
    ...
    #
    #   (a) 走査電流での Vf … 行に届くのは（レール − Vf）で
    #       nRF52840 の VIH は 0.7×レール。レール ≧ Vf/0.3 が要る
    #   (b) **逆漏れ … ダイオード本来の役目（ゴースト防止）**。
    #       プルダウン 13kΩ に対し、同じ行の同時押し n 個ぶんが足し算になる
    #
    #   型番      (a) Vf の保証        (b) 逆漏れの保証   単価/305個
    #   1N4148W   **なし**(1mA 0.715V) 25nA @ 20V         $3.57 Basic ← これ
    #   SD103AW   0.37V @ 20mA         **なし**(30V 5µA)  $3.54 +$3
    #   BAT46W    250mV @ 0.1mA        0.5µA @ 1.5V       $28.1 +$3
    #
    # **一度 SD103AW に替えたが戻した。**Vf の不定を逆漏れの不定に
    # 取り替えただけで、しかも逆漏れ側の失敗は**ゴースト＝誤った文字**。
    # 保証値 5µA を当てると 2 個同時押しでも margin を割る。
    #
    # 1N4148W は逆漏れが桁違いに小さく、**本来の役目は保証される**。
    # 不定なのは打ち止めだけで、これは「電池が早く終わる」であって誤動作
    # ではない。100µA での Vf を実測すれば閉じる（open-gaps #25）。
    #
    # BAT46W は両方を動作点で保証する唯一の候補だが **+$24.5（¥3,700）**。
    # 実測が難しいと分かったらここへ戻る。
    "diode":     {"lcsc": "C81598",
