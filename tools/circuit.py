...
# BLE 送信中の電圧降下。**マイコンの余裕 MCU_MARGIN がこれを覆う必要がある。**
#
# 降下は「電流 × 電池の内部抵抗」で決まる。**バルクコンデンサでは消せない。**
# コンデンサが効くのは τ = R×C より短い間だけで、送信バースト（数百µs）に
# 対して τ は 40〜150µs しかない。**容量を倍にしても降下は 3mV しか変わらない。**
BLE_TX_CURRENT = 0.015       # [暫定] 送信中の消費電流。Task C4 で実測する
BATT_ESR_END = 1.5           # [暫定] 電池末期の内部抵抗（2 本ぶん合計）
