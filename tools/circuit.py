...
#   **起動の限界**（切って入れ直す）      **1.90V ＋ 0.030 × 内部抵抗[Ω]**
#                                         10Ω で 2.20V / 25Ω で 2.65V（実測）
#
# **打ち止めを決めるのは起動のほう。**起動は走査・広告で 2.7mA、瞬間は
# 30mA 級を引くので、内部抵抗の影響を 10 倍以上強く受ける。
# **スリープ復帰も起動と同じ手順を踏む。**
#
# 電池の電圧と起動の限界が交わるのは **2.17V 付近**（内部抵抗と電圧は
# 放電曲線の上で一緒に動く）。**打ち止め 2.40V はその 0.23V 手前。**
#
