...
#
# ⚠️ **単独で下げてはいけない**（2026-08-11 に判明）。打ち止めは
# `_RAIL_FLOOR + SCHOTTKY_VF` なので、実測値 0.18V に直すと打ち止めが
# 2.18V になり、**起動の限界 2.20V（10Ω・実測）を下回る。**
# 装置が「自分で再起動できない電圧まで使い切ってから止まる」ことになる。
# → 打ち止めの定義ごと見直す話。**open-gaps #34。**
...
# → 打ち止め 2.40V に対して **余裕 0.15〜0.25V。潤沢ではない。**
#
