...
#
# **SD103AW の保証値を使う。**データシートは VF ≦ 0.37V @ 20mA。
# 実使用は 100µA でそれよりはるかに小さいので、0.37V は安全側。
# **実測を待たずに保証で詰められる**ので暫定値ではない。
#
# 1N4148W（シリコン）だと 1mA 未満の最大値が規定されておらず、
# 1mA の最大 0.715V で詰めるとレール 2.55V（1.48V/本）になっていた。
MATRIX_DIODE_VF = 0.37   # SD103AW の保証値（20mA での最大）
