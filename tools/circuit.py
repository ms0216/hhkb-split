...
def netlist(half):
    ...
    parts = [
        ...
        # レールが基板を出入りするところ（J_DB＝子基板への FFC）の高周波を落とす。
        #
        # **以前の名前は C_MCU だったが、この基板に MCU は無い**（XIAO は
        # 子基板側で、そちらには C_DB がある）。名前が役割を偽っていたので
        # 2026-08-12 に改名した。実体は「レールの入口のパスコン」。
        # FFC は 100mm あり、その先に BLE の送信機——この系で最も騒がしい
        # もの——がぶら下がる。ケーブルから戻る高周波をここで落とす。
        ("C_RAIL", "cap_100n", {"1": "V3V3", "2": "GND"}),
