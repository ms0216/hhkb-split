...
#
# **1N4148W は 1mA 未満の Vf 最大値を規定していない。**1mA の最大 0.715V で
# 詰めるとレール 2.55V（1.48V/本）になる。典型 0.5V なら 1.83V。
# **桁が変わるので実測でしか決まらない**（open-gaps #25）。
#
# ショットキー（SD103AW）に替えれば Vf は保証で詰められるが、**逆漏れの
# 保証が動作点に無い**（30V で 5µA のみ）。それを当てると 2 個同時押しでも
# ゴーストの margin を割る。**役目が逆転するので替えない。**
# 両方を動作点で保証するのは BAT46W だけで、+$24.5 かかる。
