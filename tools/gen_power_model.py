...
LEVERS = (
    ...
    ("送信電力を 0 → +8dBm",
     f"＋{tx8_ma(7.5,15,30):.3f}mA", "−1.1 ヶ月",
     "電波の余裕を買う（左だけ強くなる）", "実測 0.54mA。ZMK の「無視できる」は当てはまらない", RED),
    ("送信電力を 0 → −20dBm",
     "−0.01mA", "＋0.15 ヶ月",
     "届く距離を捨てる", "実測 0.43mA。**電力 1/100 でも 1〜1.5m で打鍵できた**", GREY),
    ("2M PHY をやめて 1M にする",
     "＋0.05mA", "−0.7 ヶ月",
     "感度が 3dB 良くなる（届きやすくなる）", "実測 0.49mA。**2M は既定で効いていた**", ORA),
...
CERT = (
    ("強い", GREEN,
     "分割リンクの係数 2.591 — 8 点の当てはめに加え、15ms と 30ms を予測してから実測して当てた"),
    ("強い", GREEN,
     "床が増えないこと — 3 本のファームで実測。差はすべて分解能以下"),
    ("強い", GREEN,
     "候補 A の 0.43mA — Mac・Windows・電池入れ直しで一致。macOS はログで受理を確認"),
    ("強い", GREEN,
     "送信の費用 — +8dBm/−20dBm/1M の 3 実測が「パケット 40µs ＋ 立ち上がり 40µs」で同時に説明できる"),
    ("弱い", RED,
    ...
    ("未確認", GREY,
     "本番基板の電波環境（アンテナ #23・左右の距離・人体）。治具では −20dBm でも 1.5m 通ったが、基板は別"),
)
