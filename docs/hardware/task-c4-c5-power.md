...
| **§7b** | **C4-4 打ち止めで自分から止まるか** | 可変電源（LM2596S でよい）。**治具の設定を 2 か所だけ変える** |
| **§8** | C5 電池残量 | テスター＋ホストの表示 |

書き込むファームは**全部を通して `proto_split_left` の 1 つ**。
**§7b だけは別**（打ち止め停止のコードが治具に入っていないため。§7b 冒頭）。
...
| スライドスイッチ | 1 | 無ければジャンパの抜き差しで代用可。**本番用（SS12D01G4-G・秋月 112723。2026-08-12 決定）を待つ必要は無い** |
...
## 7b. Task C4-4: 打ち止めで**自分から止まる**か（open-gaps #25b の実機確認）

**§7 が測ったのは「どこまで動くか」。ここで測るのは「動かなくなる前に
自分で止まるか」。**別の項目で、まだ一度も実機で動かしていない。

> **ファームは書けているがビルドが通っただけ。**
> [firmware/src/low_battery_off.c](../../firmware/src/low_battery_off.c) の
> 冒頭にもそう書いてある。**LM2596S があればここで閉じられる。**

### まず ⚠️ この治具のままでは試験できない

**`proto_split` は打ち止め停止のコードを含んでいない。**

```
proto_split.dtsi   compatible = "zmk,battery-voltage-divider"   ← ZMK 標準
hhkb_split.dtsi    compatible = "hhkb,battery-alkaline"         ← 本番
```

`low_battery_off.c` は `empty-millivolts` を持つ `hhkb,battery-alkaline`
が前提で、`HHKB_LOW_BATTERY_SOFT_OFF` は
`depends on HHKB_BATTERY_ALKALINE`（[firmware/Kconfig](../../firmware/Kconfig)）。
**治具では丸ごとコンパイルから外れる。**焼いて「止まらなかった」を見ても、
それは**機能が入っていないだけ**で、何も測っていない。

→ **試験用の設定を治具に足す。**ハードは §4 のまま、触るのは 2 ファイルだけ。

| 直すもの | 何を |
|---|---|
| `proto_split.dtsi` の `vbatt` | `compatible` を `"hhkb,battery-alkaline"` にし、`empty-millivolts = <2400>` / `full-millivolts = <3300>` を足す（**本番と同じ値**） |
| `proto_split_left.conf` | `CONFIG_ZMK_BATTERY_REPORT_INTERVAL=5` を足す（**試験用。既定 60 秒 × 2 回 = 最短 2 分待ちを 10 秒にする**） |

**⚠️ どちらも試験が終わったら戻す。**特に `REPORT_INTERVAL` を戻し忘れると
製品が 5 秒ごとに ADC を回す。`empty-millivolts` は本番と同じ値なので
戻し忘れても実害は無いが、**治具と本番で違う残量計を使う理由（§8 の
`full-ohms` 2.5 倍の手）が壊れる**ので戻す。

### ⚠️ ログを読むために USB を挿すと、試験にならない

`low_battery_off.c` の判定は `LOG_WRN` / `LOG_ERR` で見えるが、
**USB を挿すと 3V3 が USB 側から給電されてレールが 3.3V に持ち上がる**
（§2 のショットキーがまさにそう働く）。**電池側をいくら下げても
打ち止めに掛からない。**

→ どちらかを選ぶ。

> **2026-09-03 追記:** `low_battery_off.c` は **USB 給電中は判定そのものを
> 飛ばす**ようになった（`zmk_usb_is_powered()`。スイッチ OFF＋USB で分圧が
> 0mV になり即 soft off するのを避けるため）。普通の USB ケーブルでは
> レールの問題以前に**判定が走らない**。ログを読むなら B（VBUS を切った
> ケーブル）しかない。また事象待ちからタイマーに変えたので、
> 下の 3 の「`zmk_battery_state_changed` が出ているか」は気にしなくてよい
> （% が変わらなくても `REPORT_INTERVAL` ごとに判定する）。

| | やり方 | 判定できるもの |
|---|---|---|
| **A（推奨）** | **ログを見ない。**BLE が切れることで判定する | 止まったかどうか。**追加部品ゼロ** |
| B | **VBUS（5V）線を切ったケーブル**を使い、`CONFIG_LOG` 一式を戻す（[proto_split_left.conf](../../config/boards/shields/proto_split/proto_split_left.conf) の L78-83 にコメントで残してある） | mV の実測値と回数まで読める |

**A で足りる。**見たいのは「打ち止めを割ったら止まるか」で、
mV は §7 のテスターで既に読めている。

**A のときは `CONFIG_HHKB_STATUS_LED=y` のままにしておく**（すでに y）。
止まる直前は「ホスト未接続 ＝ 青点滅」、soft off に入れば**完全に消灯**する
ので、目で切り分けられる。

> ⚠️ **§6 の「電流を測るときは LED を外す」はここでは当てはまらない。**
> C4-4 で測るのは電流ではない。LED は点けたまま使う。

### 手順

**§7 のつなぎ方（電池ボックスを外して LM2596S を 29 列 a 行 / 12 列 a 行）を
そのまま使う。**シャントは **10Ω**（1kΩ だと降下で起動できない・§6 の警告 1）。

> **⚠️ LM2596S をつなぐ前に、§7 の「可変電源は、つなぐ前に必ず出力を合わせる」
> の 6 手順を必ず通すこと。**出荷時のトリマは不定で、**5V が出ていれば
> XIAO は一発で壊れる**（nRF52840 の絶対最大 3.9V）。

1. 試験用の設定（上の表）を入れてビルドし、`proto_split_left` を焼く
2. LM2596S を **3.00V** に合わせ、つなぐ。USB は**抜いたまま**
3. ホストとペアリングし、**繋がっていること**を確認する（LED 消灯）
4. **2.50V** まで下げる。まだ打ち止め 2.40V の上なので、**止まらないこと**を確認する
   — ここが対照実験。**「止まった」だけを見ると、電圧と無関係に止まる不具合を
   見逃す**
5. **2.30V** へ下げる（打ち止め 2.40V を割る）
6. **10 秒待つ**（5 秒 × 2 回）。BLE が切れ、LED が消えるはず
7. **電源を 3.00V に戻す。**ここで**勝手に復帰しないこと**を確認する
   — soft off は電源の入れ直しでしか起きない（`low_battery_off.c` の「復帰」）
8. **スイッチを切って入れ直す。**起動して再びホストにつながることを確認する

### 判定

| # | 見るもの | 期待 |
|---|---|---|
| 1 | 2.50V（打ち止めの上） | **止まらない**（対照実験） |
| 2 | 2.30V（打ち止めの下）で 10 秒 | **切れる・LED 消灯** |
| 3 | 電圧を戻しただけ | **復帰しない** |
| 4 | 電源を入れ直す | **起動する** |

**4 つ揃って合格。**2 だけを見て合格としないこと。

### ⚠️ 1 が落ちたとき（2.50V でも止まる）に疑うもの

**ADC の読みがずれている可能性が高い。**§8 の「% を電圧計に変える」手で
ADC の実際の読みを逆算し、テスターの読みと突き合わせる。
分圧の実測（R1 976kΩ / R2 987kΩ）はすでに取ってある。

### ⚠️ 2 が落ちたとき（2.30V でも止まらない）に疑うもの

順に。

1. **焼いたファームに機能が入っているか。**`.uf2` の大きさが変わっていない
   なら、設定が効いていない（C2 の教訓「焼いたファームがどのコミットのものか
   確かめる」）
2. **`REPORT_INTERVAL` が効いているか。**10 秒ではなく 2 分待ってみる
3. **既に切れていないか。**2.30V はレール 2.11V で、§7 の「維持の限界
   1.95V」よりは上だが、**もし先に BLE が落ちていると
   `zmk_battery_state_changed` が出ているかどうか自体が確かめにくい**
   → やり方 B（VBUS を切ったケーブル＋ログ）に切り替える

### まだ閉じないもの

**この試験は「打ち止めの判定と停止が動く」ことしか見ない。**
**電池を実際に使い切ったときに同じことが起きるかは別**（可変電源は内部抵抗が
ほぼゼロで、電池は下がりながら内部抵抗が上がる）。§7 の「10Ω での起動の限界
2.15〜2.25V」と打ち止め 2.40V の余裕 0.15〜0.25V が効くのはそちら。
→ [open-gaps #33](open-gaps.md)（温度補正）と併せて、実機の基板で測り直す。
