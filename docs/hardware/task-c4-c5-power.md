...
| 直列抵抗 | 起動できる最低電圧 |
|---|---|
| ~0Ω（可変電源のみ） | 2.16V で起動した（それ以下は未確認） |
| **10Ω** | **2.15〜2.25V** |
| **25Ω**（100Ω×4 並列） | **2.60〜2.70V** |

```
**起動の限界 ≒ 1.90V ＋ 0.030 × 内部抵抗[Ω]**   （2 点から）
```

**傾き 0.030V/Ω は、起動の瞬間に 30mA 級を引いていることを意味する**
（平均 2.7mA の 10 倍）。バルクの充電と無線の立ち上がりと思われる。

#### この式が、今日の全部を説明する

| | 内部抵抗 | 式の予測 | 実際の電圧 | 観測 |
|---|---|---|---|---|
| 可変電源のみ | ~0Ω | 1.90V | 2.16V | **動いた** |
| Panasonic | 2.68Ω | 1.98V | 2.69V | **動いた** |
| **三菱** | **9.18Ω** | **2.18V** | **2.16V** | **動かなかった** |

**三菱は予測 2.18V に対して 2.16V。20mV 足りずに落ちていた。**

#### なぜ 2 つの限界が違うか

```
維持    すでにリンクがある。**0.44mA**
起動    これから探して繋ぐ。走査・広告で **2.7mA**、瞬間は 30mA 級
        → **内部抵抗の影響を 10 倍以上強く受ける**
```
...
```
電池の電圧 と 起動の限界 が交わるのは **2.17V 付近**
  （2.69V/2.68Ω では余裕 0.71V、2.16V/9.18Ω では −0.02V。この間で交差）

設計の打ち止め **2.40V**  →  **交点より 0.23V 手前で止めている。足りている**
```
