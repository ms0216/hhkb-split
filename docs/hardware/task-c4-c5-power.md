...
**起きているときの電流が、右半分不在のスキャンで水増しされている。**
参考として両端だけ示す。

| 起きているときの電流 | 平均（8.5h/日） | 寿命 |
|---|---|---|
| 3.7mA（右半分不在。**実機ではない**） | 1.31mA | 2.1 ヶ月 |
| 1.0mA（スキャンが止まった場合の仮定） | 0.36mA | 7.7 ヶ月 |

**仕様書の見込みは 3〜8ヶ月。**満たすかどうかは、C3 で両半分をつないだ
状態の電流で決まる。**基板の発注はせき止めない**（電流で基板は変わらない）。
