...
2. 測定点 **Ⓐ（29 列 e 行 ↔ Ⓔ 12 列 d 行）** で電池電圧を読む
3. 測定点 **Ⓒ（14 列 a 行 ↔ Ⓔ）** でショットキー後のレール電圧を読む
...
> **種類は結論に効かない。**Ⓐ は同じ負荷をかけた電池端子で測っているので、
> 内部抵抗はすでに Ⓐ に織り込まれている。しかもマンガンは 1 本 1〜2Ω と
...
|---|---|---|
| Ⓐ 電池電圧 | **3.00V** | |
| Ⓒ レール（ショットキー後） | **2.75V** | ✅ 下限 1.80V に対して余裕 0.95V |
| BLE で見えるか | **見えた** | ✅ |

**✅ C4-1 は合格。** 乾電池 3.0V で ZMK が動く。

降下 0.25V の内訳は、同じ配線のまま Ⓑ を読んで分けた（測定 C）。

| シャント | Ⓑ の降下 | 電流 | Ⓒ レール | 差引 **Vf** |
|---|---|---|---|---|
...
**レールが落ちているかどうかもオシロなら一目。**1kΩ で Ⓒ が 1.35V に
...
4. **測定点 Ⓑ（29 列 b 行 ↔ 27 列 b 行）**を、**赤を 29 側**にして測る
...
→ **USB を抜いて、同じ Ⓑ をもう一度読む。**1kΩ 越しに電池から引くので
...
| 候補 | 内容 | 見分け方 |
|---|---|---|
| A | **ショットキーの逆漏れ。**レール 3.3V − 節点 A 3.0V = 0.3V の逆バイアスで 0.7µA ほどが充電の向きに流れ、放電 1.5µA と相殺する | Ⓓ が Ⓐ の半分なら、こちら |
...
テスターの内部シャントは µA レンジでも 100Ω 級なので、起動時 2.2mA でも
降下は 0.2V ほど。レールは 2.5V 台に残り、起動できる。
...
1. **29 → 27 列（c 行）のシャントを 10Ω にする**。他はそのまま
2. **測定点 Ⓑ** を読む。無操作のときと、キーを連打したときの両方

#### 実測結果（2026-08-09・無操作）

| | 想定 | 実測 | |
|---|---|---|---|
| BLE 接続中・無操作 | 5〜15mA | **20〜25mV → 2.0〜2.5mA** | 想定より軽い |

**この 2.2mA はスリープ電流ではない。**このとき焼いていたファームには
`CONFIG_ZMK_SLEEP` が無く、deep sleep に入らなかった（上の警告 1b）。

**このまま下がらなければ電池寿命は 2000mAh ÷ 2.2mA ≒ 38 日**で、
仕様の 3〜8ヶ月に届かない。**測定 B がこのタスクの山場になる。**

### 電池寿命の見積もり

アルカリ単3 の容量は約 2000mAh。

### 実測から出した見積もり（2026-08-09）
...
| 起きている時間 | 平均電流 | 寿命 |
|---|---|---|
| 8.5h/日 | 0.78mA | **3.5 ヶ月** |
| 4h/日 | 0.37mA | 7.5 ヶ月 |

**仕様書の見込み 3〜8ヶ月に入る。** ただし**下限に近い**。

**スリープ電流を削っても寿命はほぼ変わらない。**分圧を 10MΩ に上げても
平均は 0.78 → 0.7787mA。**効くのは起きているときの 2.2mA だけ。**

### ⚠️ その 2.2mA の測定条件に疑いがある（未解決）

**測定 C を取ったのは起動から間もない時点だった。**ZMK は
`CONFIG_ZMK_IDLE_TIMEOUT`（既定 30 秒）を過ぎると idle 状態へ移り、
BLE の接続間隔を伸ばして消費を下げる。**起動直後は広告の頻度も高い。**

つまり **2.2mA は「起きているときの定常値」ではなく「起動直後の値」の
可能性がある。**寿命を決める唯一の項目なので、ここは詰める価値がある。

**追試のやり方。**治具の `CONFIG_ZMK_IDLE_SLEEP_TIMEOUT` を本番と同じ
1800000（30 分）に戻して焼き、**接続したまま 2〜5 分放置してから**
10Ω で測る。定常値が 2.2mA より小さければ、寿命の見込みは伸びる。

**基板の発注はせき止めない**（電流の値で基板は変わらない）。
