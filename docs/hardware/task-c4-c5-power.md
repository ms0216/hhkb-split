...
数値（1A で 0.6V・40V）も同じ。**ここで測った Vf は B5819W の値として
使える。買い足しは要らない。**
...
| 単3電池 | 2 | |
| **1N5819**（ショットキー） | 1 | 手持ち。**本番の B5819W と規格が同じ**（1A 40V）なので、測った Vf をそのまま設計値に使える |
...
## 5.9. ついでに測る: マトリクスのダイオードの Vf（**5 分**）

**これが打ち止めを決めているのに、データシートに規定が無い。**

行に届くのは（レール − Vf）。nRF52840 の VIH は 0.7 × レール。つまり
**レール ≧ Vf ÷ 0.3**。`1N4148W` は 1mA 未満の Vf 最大値を規定していない。
1mA の最大 0.715V で詰めるとレール 2.55V（1.48V/本）、典型 0.5V なら 1.83V。

> **⚠️ `1N4007` では測れない。**C2 で使ったのはそちらだが、接合が大きく
> Vf が低く出るので**楽観的な値になる**。**`1N4148` が要る**（本番は
> その表面実装版 1N4148W）。手元に無ければ、この測定は飛ばしてよい。
> その場合は打ち止めを保守的に置くか、BAT46W（両方を動作点で保証。
> ただし +¥3,700）に替える。

### 測り方

1. 電池（3.0V）＋ 抵抗 ＋ **1N4148** を直列にする
2. **ダイオードの両端**の電圧を測る（＝ Vf）
3. **抵抗の両端**も測る。電流 ＝ その電圧 ÷ 抵抗値
4. 抵抗を替えて 2〜3 点。**10µA・100µA・1mA あたり**（270k / 27k / 2.7kΩ 相当）

| 抵抗 | 抵抗の両端 | 電流 | **Vf** | 部品の確認 |
|---|---|---|---|---|
| 約 270kΩ | V | µA | **V** | — |
| 約 27kΩ | V | µA | **V** | — |
| 約 2.7kΩ | V | µA | **V** | **1mA なら 0.715V 以下** |
| 約 270Ω | V | mA | **V** | **10mA なら 0.855V 以下** |

### 測りながら「本当に 1N4148 か」を確かめる

刻印が読めなくても、値で分かる。

| 見えたもの | 判定 |
|---|---|
| 1mA で 0.6〜0.72V、10mA で 0.7〜0.86V | **1N4148 として矛盾しない** |
| どの電流でも 0.2〜0.4V 台 | **ショットキー。**この測定には使えない |
| 1mA で 0.5V 前後と低い | 整流用（1N400x 系）。**使えない** |
| 逆向きにしても電流が流れる | ツェナー。**使えない** |

### 合否の線（**測る前に決めておく**）

余裕 +0.05V を足した値が **0.49V** を超えると、ダイオードが打ち止めを
決めるようになる（それ以下ならマイコンが律速で、ダイオードは無関係）。
つまり**実測値が 0.44V 以下なら、1N4148W で何も失わない。**

| 実測（100µA） | +0.05V | 打ち止め | 判断 |
|---|---|---|---|
| 〜0.44V | 〜0.49V | **1.10V/本** | **そのまま。ダイオードは律速でない** |
| 0.45〜0.49V | 0.50〜0.54V | 1.12〜1.18V/本 | 許容範囲。そのまま進む |
| 0.50〜0.55V | 0.55〜0.60V | 1.20〜1.28V/本 | 電池の末尾を捨てる。**要相談** |
| 0.55V〜 | 0.60V〜 | 1.28V/本〜 | **BAT46W へ替える**（+¥3,700） |

BAT46W は VF ≦ 250mV @ 0.1mA と IR ≦ 0.5µA @ 1.5V を**どちらも動作点で
保証**する唯一の候補。替えればこの表は不要になる（打ち止めはマイコン律速の
1.10V/本 で確定）。

### 入れる値

**測った 100µA の値に +0.05V 足して `tools/circuit.py` の
`MATRIX_DIODE_VF` に入れる。**打ち止め `BATT_V_MIN` は自動で決まる。

+0.05V は**温度と個体差の余裕**。1 本測っても出るのは典型値であって
保証値ではない。本番は 61 個 × 2 枚あり、製造元も違う（本番は Semtech の
1N4148W、手元のものは別ベンダの可能性が高い）。**「測ったから確定」には
ならない**ので、暫定値の一覧には残したまま扱う。

## 6. Task C4-3: どこまで電圧が下がっても動くか

> ⚠️ **ここで出る値は「マイコンの限界」だけ。**この治具にはマトリクスが
> 無いので、ダイオードの分は含まれていない。**そのまま打ち止めにしない。**
> 実機の打ち止めは `tools/circuit.py` が 3 つの制約（マイコン・IC の
> 動作範囲・マトリクス）の最大値から計算する。
