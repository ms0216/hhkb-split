...
（レール − ダイオードの Vf）。nRF52840 の VIH は 0.7 × レール。つまり

    レール − Vf ≧ 0.7 × レール   →   **レール ≧ Vf ÷ 0.3**

`1N4148W` のデータシートは **1mA 未満の Vf 最大値を規定していない**。
1mA の最大 0.715V で詰めるとレール 2.55V（1.48V/本）が必要になり、
典型 0.5V なら 1.83V。**桁が変わるので実測でしか決まらない。**

### 測り方

手持ちの `1N4148`（C2 で使ったもの）と抵抗 1 本でよい。

1. 電池（3.0V）＋ 抵抗 ＋ ダイオード を直列にする
2. **ダイオードの両端**の電圧を測る（これが Vf）
3. **抵抗の両端**の電圧も測る。電流 = その電圧 ÷ 抵抗値
4. 抵抗を替えて 2〜3 点取る。**10µA・100µA・1mA あたりを狙う**
...
手持ちの値でよい。**電流は計算で出せる**ので、きりのいい値でなくて構わない。
...
**100µA での Vf を `tools/circuit.py` の `MATRIX_DIODE_VF` に入れる。**
そこから打ち止めが自動で決まる（`BATT_V_MIN`）。

温度でも動く（冷えると Vf は上がる）。**余裕を見て、測った値に +0.05V**
くらい足した数を入れるのが安全。
