...
|---|---|---|
| 定格 | 2A 40V | 1A 40V |
| 10〜15mA での Vf | **これから測る** | **測れない**（手元に無い） |
...
- **C4 でこのダイオードを使ってよいのは、構成の確認（動くか・逆流しないか・
  充電しないか）まで。**そこはどちらのショットキーでも同じ結論になる
- **Vf の実測は B5819W を手に入れてから。**LCSC で $0.03。基板と一緒に
  注文するか、国内で 1N5819 系（同等品）を買えばよい
- それまで `SCHOTTKY_VF = 0.4` は保守値のまま据え置く
  （B5819W のデータシートは 1A で 0.6V しか保証しておらず、
  特性グラフも 0.1A まで。10〜15mA は図の外）
...
`proto_split_left` か本番の `hhkb_split_left` を書き込んでおけば、
Mac の Bluetooth 設定に現れるはず。
...
シールドは部品が届いてから、実測値に合わせて用意する。抵抗の実測値
（1MΩ と銘打っていても誤差 5% はある）をテスターで測って入れると精度が上がる。

### 判定

| 項目 | 期待 | 実測 |
|---|---|---|
| ホスト側に電池残量が表示される | されること | |
| 表示値とテスター実測の一致 | ±5% 以内 | |
