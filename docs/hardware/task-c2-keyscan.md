...
**1 枚に収めることはできない。** ブレッドボードは 30 列で、1 枚目は
XIAO が 1〜7 列・列バスが 8〜9 列・スイッチとダイオードが 10〜24 列を使う。
空きは 25〜30 列の **6 列**しかなく、DIP-16 の 595 には **8 列**要る。
...
**XIAO の 3V3・GND は上側のピン（h 行）**で、本体が g・f・e 行を塞ぐので、
その列で空いているのは **i 行と j 行の 2 穴**。VCC と MR、OE とピン 8 の GND
はどちらも 2 か所へ配る必要があるが、**板をまたぐ線を増やさないほうが確実**
なので、2 枚目の側にバスの列を作って分岐させた。上の図では 3V3 を
2 列の上半分、GND を 3 列の下半分に置いた。

> **2026-08-08 に直した。**ここには「空きが 1 穴ずつしかない（ピンは i 行）」
> と書いてあったが、それは**配線図が XIAO を i 行/c 行（8 ピッチ）に
> 描いていた誤り**から来ていた。ピン間隔は 0.6 インチ＝6 ピッチなので
> **h 行と d 行**（[§4](#4-ピンヘッダの半田付け)）。図を直した。
> **実際の配線は 1 本も変えていない**ので、C2-b の実測結果はそのまま有効。
...
### 実測結果（2026-08-08・4MHz）

| スイッチ | 期待 | 実測 | 判定 |
|---|---|---|---|
| a（10・12 列） | a | a | ✅ |
| b（14・16 列） | b | b | ✅ |
| c（18・20 列） | c | c（6 押しで 8 文字） | ✅ 文字は正しい |
| d（22・24 列） | d | d | ✅ |

a→b→c→d を 6 巡させた生の出力は `abcdabccdabcdabcdabcdabccd`。
**重複を 1 つに潰すと `abcd` × 6 に完全一致**し、順序も毎回 a→b→c→d だった。
**取り違えもゴーストも無い。**

`c` の重複はチャタリング（[open-gaps.md](open-gaps.md) の 25）。
**ゴーストなら違う文字が出る。同じ文字の重複は機械的バウンス**なので、
合否には影響しない。
...
**転送速度は本番と同じ 4MHz に揃えてある**（`proto_shift.overlay`）。
以前はここだけ 200kHz だったが、それでは**通っても本番の保証にならない**。
取りこぼしは転送速度で決まるので、確かめたい条件そのもので測る必要がある。
`test_the_prototype_shield_uses_the_production_spi_speed` が食い違いを見張る。

取りこぼしが出たら、**逆に下げて**切り分ける（2MHz → 1MHz）。
速度で直るなら転送の問題、変わらないなら配線か接触の問題。
どちらだったかを記録欄に書くこと。

#### 実測結果（2026-08-08・4MHz）

`a` と `b` を 20 往復。出力は **40 文字ちょうどの `ab` × 20**。
**連続同一文字は 1 つも無い。取りこぼしゼロ・余分ゼロ。**

ZMK 公式が警告する取りこぼしは、**1 個・数珠つなぎなし・4MHz では起きない**。

**一度 200kHz のファームで測ってしまい、やり直した。** 手順書に
「確かめたい条件そのもので測れ」と書いたそばから、コミット
`9dae4cc`（4MHz へ変更）より前のビルドを焼いていた。
**焼いたファームがどのコミットのものかを確かめること。**
...
**実測で確認した（2026-08-08）。** 入れ替わりは起きず、
`col-gpios` は現状の順序のままでよい。

### 組んだときに引っかかったこと

**全キー無反応から始まった。原因は STCP（12 番）の接触不良**だった。
ラッチのクロックが来ないので、ビットはシフトレジスタに入るのに出力へ
移らず、QA・QB が永久に 0V になっていた。

**紛らわしかったのは、SPI 書き込みが成功を返し続けること。** nRF 側で転送は
完結するので `spi_write_dt()` は 0 を返す。ドライバから見ると正常に見える。

**効いた切り分け**

1. **QA・QB の待機電圧を測る。** ZMK は割り込みを待つ間、全列を High に
   するので、**何も押していないとき QA は 3.3V のはず**。0.00V なら
   595 まで届いていない。**これが一番速い**
2. 595 を経由せず、**3V3 を列バスへ直結**して押す。文字が出れば
   1 枚目・行・ファームは無事で、故障は 595 の出力までの間に絞れる
3. テスターに導通モードが無い場合、**線の片端を 3V3 バスに挿し替えて
   反対側のピンを測る**。3.3V が出れば線と穴は正常。導通モードの代用になる

## 9. 判定

**✅ C2-a は 2026-08-07 に合格した。**
**✅ C2-b は 2026-08-08 に合格した（4MHz・本番と同じ転送速度）。**

列を 74HC595 で駆動しても、対応表どおりの文字が出て、速い打鍵でも
取りこぼさない。方式は成立。フェーズ D（KiCad での基板設計）へ進んでよい。
