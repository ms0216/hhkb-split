...
| **候補 A（15ms・latency 30）** | **0.43mA** | **10.5 / 14.25ms** | **6.5 ヶ月** |

**遅延は 1ms も増えていない。分割リンクにも触っていない。左右差も同じ。**

###### ⚠️ ただし、ペアリングができない（**製品としては致命的**）

**ボンドが無い状態からは、macOS とペアリングできなかった。**

| 状態 | 結果 |
|---|---|
| **ボンドがある**（基準でペアリング済み → 候補 A を焼く） | **繋がる。`c` `d` も打てる** |
| **ボンドが無い**（`settings_reset` の直後） | **「接続」を押しても繋がらない** |

**対照を取って確かめた。**同じ状態で基準を焼くと、**ペアリングできて 0.63 / 0.49mA も再現した。**
→ **環境ではなく、候補 A の設定が原因。**
