...
| | 分かること |
|---|---|
| **ホストが許可した interval と latency** | 要素 4 が要るか。遅延の鎖の未知が埋まる |
| **分割リンクの PHY が steady state で 1M か** | 要素 2 が生きているか |
| **7.5 → 15 → 30ms の体感と電流** | 要素 1 の交換レート |

**上 2 つは左に `CONFIG_ZMK_USB_LOGGING=y` を入れて BLE で繋ぐだけで読める。**
`le_param_updated` がホスト側のパラメータを、
`split_central_process_connection` が分割側の PHY を出す。
