...
| # | どこに | 何を書いたか | 何で確かめたか |
|---|---|---|---|
| **A** | `proto_split_left.conf` のコメント | **「ZMK は idle で BLE の接続間隔を伸ばして消費を下げる」** | **何も。**測っていない |
...
| **D** | **分割リンク 7.5 / 15 / 30ms の目隠し比較**（第 2 段階） | 要素 1 の交換レート。**体感と電流の両方が未測定。**手間は最大 |
...
> **⚠️ この測定より先に open-gaps #29（組み立て検査に入っていない実物）を片づけること。**
> 構造の見直しで配置が動くと、ここで測った値は使えなくなる。
...
|---|---|---|---|
| **A** | XIAO 単体 | 原点 | ― |
| **B** | **＋ GND のパッドに 100mm のリード線をはんだ付けし、アンテナの脇を通して伸ばす** | **★ いちばん知りたいこと。**FFC ケーブルが共振してアンテナを引っぱるか | リード線 |
...
| 見るもの | 判断 |
|---|---|
| **B の差がふらつきの幅以内** | **共振していない。**いちばん大きな未知が消える → **発注してよい** |
| **B の差が 10dB 以上** | **共振している。**線の引き回しか、子基板の改版（#23 の「残る手」3 案）を先に決める |
