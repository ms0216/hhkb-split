...
| **[#35](#35-重大-電池蓋が外から開かない電池交換のたびに分解することになる)** | **電池蓋が外から開かない**（2026-08-12 発見） | 座ぐりの向き・留め方を決めて刷り直す。**刷る前なら費用 0** |
...
| **電池蓋** | レール（左右）＋ 手前のストッパー | **外から開かない**（#35） |
| 電池ボックス | 仕切り壁・側壁・蓋・基板で囲う（ネジ無し） | 蓋が開かない以上、動かない。#35 を直すとき一緒に見る |
| チルト脚 | φ4 × **2.4mm** のピン圧入 | **保持力は未確認**（`FOOT_PEG_H` は［暫定］。刷って確かめる） |
| 電源スイッチ | 壁のポケットへ落とし込み（**耳が無い部品**） | **内側が開いている。**押し込む向きに留めが無い——スライド操作は横向きなので実用上は動きにくいが、**組み立て時に落ちる。**接着かリード線の張りで持たせることになる。**要確認** |
| ゴム足 | 座ぐり＋粘着 | 市販品 |
