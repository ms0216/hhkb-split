...
## 25. 電池が尽きるときの止まり方（**ファームで直せる。基板は関係ない**）
...
## 27. **電源を入れる順序で、左右がつながらないことがある**

**右 → 左 の順に入れると、つながらないことが多い**（2026-08-09 実測）。
**左 → 右 なら今のところ 100% つながる。**

| 順序 | 結果 | 右の消費電流 |
|---|---|---|
| **左 → 右** | 100% 接続。`c` `d` が打てる | **40µA** |
| **右 → 左** | **つながらないことが多い**。`c` `d` が打てない | **1mA**（広告し続けている） |

**実機は左右それぞれにスイッチが付く。利用者はどちらから入れるか分からない。**
このままだと「電源を入れたのに右半分が効かない」が起きる。

### 分かっていること（ZMK の実装）

**ペリフェラルは、一度ボンドすると「相手を名指しした広告」に切り替わる**
（[peripheral.c](https://github.com/zmkfirmware/zmk/blob/main/app/src/split/bluetooth/peripheral.c) の `start_advertising`）。

```c
adv_param = low_duty ? *BT_LE_ADV_CONN_DIR_LOW_DUTY(&central_addr)
                     : *BT_LE_ADV_CONN_DIR(&central_addr);
...
if (err == BT_HCI_ERR_ADV_TIMEOUT && enabled) {
    low_duty_advertising = true;      // 高デューティは 1.28 秒で打ち切られる
    k_work_submit(&advertising_work);
}
```

**高デューティの名指し広告は Bluetooth の仕様で 1.28 秒で打ち切られ**、
低デューティへ落ちる。セントラル側は名指し広告も扱える
（[central.c](https://github.com/zmkfirmware/zmk/blob/main/app/src/split/bluetooth/central.c) の
`split_central_device_found` が `BT_GAP_ADV_TYPE_ADV_DIRECT_IND` を拾う）。

### 分かっていないこと（**推測で閉じない**）

**「1.28 秒で打ち切られるから」だけでは説明しきれない。**低デューティでも
100〜150ms 間隔なので、走査が 50% デューティなら数秒で拾えるはず。
別の要因（アドレスの解決・プライバシー・走査の開始時刻）が絡んでいる可能性がある。

### 切り分け（未実施）

| # | 試すこと | 分かること |
|---|---|---|
| 1 | 右を入れて**すぐ**（1 秒以内）に左を入れる | 1.28 秒の打ち切りが原因か |
| 2 | つながらないまま **1〜2 分**待つ | 遅れてつながるだけなのか、永久に駄目なのか |
| 3 | つながらない状態で**左だけリセット** | 左の走査の開始時刻の問題か |
| 4 | つながらない状態で**右のキーを押す** | ペリフェラル側の状態遷移で直るか |

**2 が最初。**「遅いだけ」なら実用上ほぼ問題にならないが、「永久に駄目」なら
出荷前に直す必要がある。

### 影響

**基板の発注はせき止めない**（ファームの問題で、基板は変わらない）。
ただし**出荷前に閉じる必要がある。**

つながらない間、右は 1mA を引き続ける（接続時の 25 倍）。
2000mAh ÷ 1mA ≒ 83 日で、[#26](#26-片方が居ないともう片方が探し続けて電池を食う) の
左側と同じ形。**両方が「相手が居ないと電池を食う」。**
