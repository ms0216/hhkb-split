...
| **[#42](#42-2026-08-15-gnd-のスタブが-bcu-決め打ちで-smd-パッドに触れていない)** | **GND のスタブが SMD パッドに触れていない**（左右 11 個とも。繋いでいたのはスタブではなくベタだった） | **潜在。いま赤ではないが直っていない。**ベタの形が変わればまた出る。素直な修正は 3 通り試して全部悪化した（#42 に記録）。**置き場所の探索を層ごとに分ける設計から** |
