...
**基板を発注する前に決めること。**あとからでは作り直しになる。
決めたら、開口とスイッチの当たりを `gen_assembly.py` の検査対象に入れる
（いまは見ていないので、また静かにずれる）。

## 18. スライドスイッチの実部品が未選定

フットプリントは KiCad の汎用形状
（`SW_DIP_SPSTx01_Slide_9.78x4.72mm_W8.61mm_P2.54mm`）のままで、
**実際に載せる部品が決まっていない。**#17 で置き場所が決まってから、
JLCPCB の在庫から寸法の合うものを選ぶ（順序を逆にすると選び直しになる）。
