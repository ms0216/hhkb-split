...
1. ✅ **GitHub Actions の二重化 → 「重複ではなく別の層」と分かった**
   （2026-08-11）。`real-shape` が実形状の全数を両側見る。`checks` の
   干渉確認は記録の bbox＋**予約の箱**の層で、安全余裕の箱 × ケースの
   組はここにしか出ない（下の「前提の穴」）。役割が違うので両方残す。
   利用者の原則「全組み合わせを・重複なく 1 回・正確に」は、
   **実形状の検査については** real-shape 1 回で成立している
