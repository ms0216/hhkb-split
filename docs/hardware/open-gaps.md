...
| # | 試すこと | 分かること |
|---|---|---|
| 1 | つながらない状態で**左だけリセット** | 直れば**左の走査の状態**。直らなければ右 |
| 2 | つながらない状態で**右だけリセット** | 直れば**右の広告の状態** |
| 3 | 両方に `CONFIG_ZMK_USB_LOGGING=y` を入れてログを見る | 広告と接続の状態遷移・失敗コードが直接読める |

**1 と 2 が先。**どちら側の問題かが 1 分で半分に絞れる。3 は焼き直しが要る。
