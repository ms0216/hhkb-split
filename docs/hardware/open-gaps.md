...
| # | 何 | 次の一手 |
|---|---|---|
| **#48** | **COL → U1/U2 を表面のレーンで戻す（作業中）**（2026-08-18〜19）。U1/U2 を J_DB の真下（左端）へ移したので SPI が **89mm → 19mm** に縮んだ代わりに、**列 8 本が基板を横断して左端へ戻る**必要が生まれた（直線距離の合計 524mm）。Freerouting に任せると COL 総長 1446mm・ビア 78 個で束が絡み、利用者が「どう見ても綺麗とは思わない」。**F.Cu はほぼ空**なので、そこに水平レーンを引く方針で合意。⚠️ **私の実装は失敗した**（DRC 42 件）——表の横断は引けたが、**着地（U1 のパッドへ入る最後の一歩）**で他のパッド（V3V3/SPI_SCK/CS）を横切り、7 本のビアが U1 の左に密集して互いに当たった。**利用者が手で 8 本引き直し**、私が間隔を 0.95mm に均等化して右端を列へ繋いだ（ビア 8 個）。実測: 表で x 78〜233 を横断できる帯は y 66.7-70.5(3.8mm) / 85.8-88.5(2.7mm) / 110.6-112.0(1.4mm) / 123.9-127.5(3.6mm) / 140.5-143.9(3.4mm)。**中央は 1.4mm しかなく 3 本しか入らない** | **左端を U1 のパッドへ着地させる。**U1 の COL パッドは x=76.695 に 0.65mm 間隔で 7 本（上から COL1→COL7）、レーンの順序は上から COL8/7/6/2/1/3/5/4 で**一致していない**ので並べ替えが要る。現物は `pcb/matrix_only/hhkb_split_right.kicad_pcb`。DRC の残り 20 件は全部 `dangling`（左端が未接続のため） |
| **#47** | **本番の `pcb/` を作り直していない**（2026-08-17）。キーマトリクスの配線は**凍結済み**——`gen_pcb.prewire_col_bus` が列のバスを引き、`autoroute` は `gen_pcb.replay_matrix` で**未配線の板からそのまま写す**（引き直さない）。左右とも全ホップが引けていて、最小の隙間 1.030mm。**しかし発注に使う `pcb/hhkb_split_{left,right}.kicad_pcb` は古いまま**（列の割り当ても U1/U2 の位置も前の版）。そのため `test_matrix_wiring_matches_the_firmware` と `test_the_routing_was_made_from_the_current_placement` が左右とも赤。**これは意図した状態**——利用者「本番配線はまだ。U1, U2 の場所や配線が決まってから」 | **U1・U2 の位置と配線が決まってから** `gen_pcb → autoroute → drc` を回して `pcb/` を作り直す。そのとき上の 2 つが緑に戻るはず。**戻らなければ写し取りが抜けている。**⚠️ 凍結が効いているか（写した配線が未配線の板と一致するか）は実測済み（左 209 個中 209 個が完全一致）だが、**本番を通した DRC はまだ取っていない** |
...
| — | `pcb/hhkb_split_right.kicad_pcb`（コミット済み）を `drc.py` に掛けると **未配線 1・via_dangling 2** が出る。`pcb/drc_right.json` の記録済み sha256 と一致する状態でこれが出る＝コミット時点から一貫してこの状態。CLAUDE.md の「3 基板とも DRC 0」という記述と食い違う（2026-08-15・BOM の Value 修正作業中に発覚。今回の Value 修正とは無関係、autoroute のやり直しでも再現する） | `tools/autoroute.py right` を何度かリトライして未配線 0 の版を作り直すか、原因（Freerouting の収束不足か DSN の制約か）を調べる |
