...
| [#27](#27-子基板と本体基板の隙間は-013mm-しかなくソケットが入らない) | XIAO を**直付けで確定**するか（＝交換できなくなる）、薄い USB-C プラグにするか | #23 と同時（どのみち子基板を作り直すため）。**薄いプラグは調査済み（2026-08-11）: 確実に 4.5mm を切るのはプラグコネクタ自作（datasheet 2.55〜2.90mm）のみ。市販薄型ケーブルは高さ未公表で要実測** |
...
2. **もっと薄い USB-C プラグを見つける**（高さ 4.5mm 級）
