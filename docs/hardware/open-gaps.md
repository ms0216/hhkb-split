...
| | |
|---|---|
| 本体 | **8.6 × 4.3 × 約4mm**（枠 10.6 × 15.6 × 15.5mm に対し大幅に余る） |
...
**データシートの標準は銀メッキ。**「Gold or silver plating available」で、
可動接点は「銀メッキまたは金」、**固定接点はブラス＋銀メッキ**、端子の
金メッキは「要問合せ」。

3.0V・1mA 未満の微小電流ではアーク放電が起きないので、接点表面の
硫化膜が破壊されずに堆積し続け、数か月〜数年で接触不良になりうる。
**金メッキならこの心配が無い。**

ただしスライドスイッチは動かすたびに接点を擦る（セルフクリーニング）
ので、定期的に入切する使い方なら銀でも実用上は保つ見込み。
**[Task C4](task-c4-c5-power.md) の実測で確かめる。**

（この点は外部の調査報告が「金メッキ」と断定していたが、データシートで
確かめたところ標準は銀だった。**報告は寸法の軸も取り違えており、
