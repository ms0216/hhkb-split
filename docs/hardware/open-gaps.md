...
## ★ 次のセッションで決めること（2026-08-09 に整理）

**この 1 つだけ。**

> **この risk を承知して発注するか。しないなら、何を変えるか。**

`tools/export_fab.py` の `_gate_antenna()` が、この節に
**「### 承知して発注する」という節が無い限りガーバーを出さない。**
門が要求しているのは 3 つ。
...
| # | 論点 | 現状 |
|---|---|---|
| **A** | **劣化量が測れていない。**20〜30dB は見積もりですらなく仮置き | **発注前に測る手段が無い**（子基板が無い） |
| **B** | 発注を 1 回にまとめる判断は動かせるか（子基板だけ先に出す） | 送料を理由に一度却下済み。**再考の余地はある** |
| C | 駄目だったときの作り直しの範囲と費用 | 手戻りは子基板 1 枚に閉じ込め済み。**金額は未算出** |
| D | 外付けアンテナを持つ別モジュール | **技適取得済みのものを探していない** |
| E | 送信電力を上げる | **桁が足りない**（4dB vs 20〜30dB）。補助にしかならない |
| F | 実使用の距離をどこまで支持するか | **設計側で仮定してはいけない**（分割の目的）。試験の条件として決める必要はある |
