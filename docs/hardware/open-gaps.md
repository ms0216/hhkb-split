...
## 16. 本体基板の電子部品が未実装（配置場所は確定）

`tools/circuit.py` に宣言済みだが基板に載っていない部品が 12 個ある
（`tools/test_conformance.py` の `PENDING`）。**これが空になるまで発注できない。**
...
### 4 層は入った
  ...
  F.Cu    列のバス・信号
  In1.Cu  GND ベタ（全面）
  In2.Cu  行の引き回し・電源  ← ここを使って FFC から各行へ降ろす
  B.Cu    ソケット・ダイオード・行のバス・部品

### 残っている手順

1. 段の間に部品を置く（`gen_pcb.py`）
2. 595 の出力を列のビアへ（F.Cu）
3. FFC から行へ（In2.Cu を経由。**2 層では通らなかったのはここ**）
4. 電源を配る（In2.Cu ／ B.Cu）
5. `PENDING` を空にし、DRC 0 を確認
6. ガーバー・BOM・CPL を出す
