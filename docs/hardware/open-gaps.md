...
1. **製品単位のまとめを記録する** → 230 秒 → 約 90 秒（6 割減）。いちばん効く
2. そのうえで A/B を判断する。90 秒なら A（1 モード統一）が現実的になる
3. **「結果の記憶」（下記の差分案）は、まだ作らない。**
   モード A は 7.8 秒、モード B も 1 を入れれば 90 秒。**仕組みを増やす価値が
