...
| ~~[#44](#44-2026-08-16-深いスリープ中は電源を切ってもすぐには切れないバルクコンデンサの電荷)~~ | ~~深いスリープ中に電源を OFF→ON してもリセットされない（53 秒）~~ | **✅ 2026-08-16 決着。バルクコンデンサ C_BULK を削除した**（実測で「守る相手が存在しない」と判明）。待ち時間は**数秒**になり、**部品も 2 個減った**。[decisions/2026-08-16-remove-bulk-cap.md](decisions/2026-08-16-remove-bulk-cap.md)。⚠️ **消したのはバルクだけ。パスコンは全部残る**（役割が別）。**残り: 実機で待ち時間を再測定する**（¥0） |
...
> XIAO は 3V3 の**供給側**で、パスコンは内蔵している（2.2µF ×5）。
> 下の「各 IC が…（**XIAO → C_DB**）」も同じ理由で誤り。
...
> 役割が違う（バルク＝電池の内部抵抗による降下・µs〜ms／
> パスコン＝IC の切り替えノイズ・ns）。#38 の「兼ねられない」は今も有効。
>
