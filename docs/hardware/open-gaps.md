...
## 26. 治具の分圧比を一時的にずらしてある（**戻す必要がある**）

**`proto_split.dtsi` の `full-ohms` を 2000000 → 2500000 にしてある**
（2026-08-09）。**本番 `hhkb_split.dtsi` は 2MΩ のまま。**

### なぜずらしたか

**2MΩ のままでは、ADC が生きているか死んでいるか区別できない。**

ZMK の残量 % はリチウムイオンの曲線で計算され、**3450mV 以下は一律 0%**
...
乾電池 2 本は 2.0〜3.2V なので**常に 0%**。

**ADC が正しく 3.0V を読んでも、壊れて 0V を読んでも、同じ 0% が出る。**
「0% が出た」は何も証明しない。BLE スキャナで生値を読んでも同じ。

比を 2.5 倍にすると報告値が 0% に潰れない領域へ入り、**% から ADC の読みを
逆算できる**（表示 P% → ADC 電圧 = (P+459)×7.5÷2.5、分解能 3mV）。

### 戻す条件

**C5 の判定欄が埋まったら `full-ohms = <2000000>` に戻す。**
戻し忘れると、次に治具を使う人が「本番と同じ値」という記述を信じて
食い違う。`test_the_production_battery_divider_matches_the_resistors`
が本番側だけを見張っている（治具側は見張れない。値が意図的に違うため）。
