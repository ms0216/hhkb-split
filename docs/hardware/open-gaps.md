...
| [#27](#27-ソケットは入る隙間-013mm-で入らないは保守平面の誤読だった2026-08-11) | ソケットか直付けか | **前提が覆った（2026-08-11）: ソケットは基板側では成立する。**要るのはケースの追随（USB 穴を座高ぶん上げて刷り直し）のみ。#23 と独立に決められる |
...
| [#27](#27-ソケットは入る隙間-013mm-で入らないは保守平面の誤読だった2026-08-11) | XIAO を**着脱可（ソケット）にするか直付けにするか**。「入らない」は誤りだったので、これは純粋な選好の判断 | いつでも決められる（#23 と独立）。ソケット化なら `DB_STACK_H` 更新＋ケース刷り直しが要る。**薄い USB-C プラグの調査（2026-08-11）はソケット可否と無関係になった**（#28・案 A 検討用の材料として節内に残す） |
...
分かりにくい壊れ方で出る。折衷案: **測定が終わるまでソケット運用 →
最後に直付けへ切り替え**（ソケットは数百円）。
