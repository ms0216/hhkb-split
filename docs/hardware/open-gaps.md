...
（電子部品の外形・ソケットの実占有域・ダイオード）＋こて先の余裕 0.6mm の内側は候補から
除外**し、他ネットの銅まで 0.5mm 未満の点も採らないようにした。その結果:

| 板 | ネット | 形 | 位置（KiCad 座標） | 他ネット／部品本体まで |
|---|---|---|---|---|
| 右 | SPI_SCK / SPI_MOSI / CS / V3V3 | ビアの裏面テンティングを外す（J_DB の右外側の縦列） | (81.7,91.9) (81.7,90.3) (81.7,91.1) (81.8,86.8) | 0.65〜0.75 / 0.72〜0.81mm |
| 右 | GND | 同 | (83.6,85.4) | 1.28 / 1.38mm |
| 右 | **U1_U2** | **無し**（隣の銅まで 0.18mm しか無い） | — | 要ビア追加 |
| 左 | SPI_MOSI / CS | 配線の上の B.Mask 窓 2.5×0.5 | (200.4,89.7) (200.8,88.4) | 1.05 / 1.34mm・0.65 / 0.90mm |
| 左 | GND | ビア | (205.1,78.1) | 2.09 / 2.02mm |
| 左 | **SPI_SCK / V3V3** | **無し**（部品から 0.6mm 以内にしか銅が無い） | — | 要ビア追加 |

**残り（利用者）**: 左 SPI_SCK / V3V3、右 U1_U2 の配線の上、部品から離れた所に
ビアを 1 個ずつ置く（`matrix_only`）。置けば規則が自動で選ぶ。それまで
`test_every_debug_net_has_exposed_copper` が露出の有無を見張る。
