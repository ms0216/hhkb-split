...
| **[#44](#44-2026-08-16-深いスリープ中は電源を切ってもすぐには切れないバルクコンデンサの電荷)** | **深いスリープ中に電源を OFF→ON してもリセットされない**（100µF が 2µA では最大 65 秒抜けない）。**電源の入れ直しという最初の復旧手段が効かない** | **まず測る（¥0）。**深いスリープに入れて OFF し、n 秒後に ON を繰り返して、実際にリセットされる最短の n を出す。**対策はその数字を見てから**（ブリーダ抵抗は常時電流と引き換え） |
...
**計算値 65 秒はリーク電流を無視した上限。**実際はレギュレータの
静止電流・コンデンサの自己放電・分圧抵抗（1MΩ+1MΩ で 1.6µA）が効くので
もっと短いはず。分圧だけで 1.6µA あるなら、それだけで 2 倍速く落ちる。

| 測ること | どうやって |
|---|---|
| **電源 OFF から、実際にリセットされるまでの秒数** | 深いスリープに入れてから OFF し、**n 秒待って ON** を n を変えて繰り返す。青が出た最短の n が答え |
| レール電圧の落ち方 | OFF した瞬間から 3V3 をオシロ／テスタで追う（`tools/gen_scope_shunt.py` の治具が使える） |
