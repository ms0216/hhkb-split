...
## 27. **右を先に入れて 10 秒経つと、左右がつながらなくなる**

**2026-08-09 に実測。電流を追って境目を出した。**

右を先に入れ、**左を入れるまでの時間 T** を変えた。判定は右の消費電流
（治具のテスターで直読。接続すると 1mA → 40µA に落ちる）。

| T | 結果 |
|---|---|
| **10 秒以内** | 左を入れた**瞬間**に 1mA → 40µA。つながる |
| **10 秒を超える** | **1mA のまま。30 秒待っても変わらない** |
| 同上・1 回だけ試行 | **60 秒待っても 1mA のまま** |
...
右を先に入れて 10 秒以上経ってから左を入れると、**右半分が丸ごと効かない。**
しかもそのあいだ右は 1mA を引き続ける（接続時の 25 倍）。

### 分かったこと

**ZMK のペリフェラルに 10 秒のタイマーは無い。**
[peripheral.c](https://github.com/zmkfirmware/zmk/blob/main/app/src/split/bluetooth/peripheral.c)
を読んだ（2026-08-09）。遅延の類（`K_SECONDS` / `k_work_delayable` /
`k_work_schedule`）は 1 つも無い。

広告の切り替えは 1 か所だけで、**高デューティの名指し広告が
Bluetooth の仕様で 1.28 秒で打ち切られたときに低デューティへ落ちる。**
...
以後、低デューティの名指し広告が続く（打ち切りは無い）。

### **1.28 秒の仮説は外れた**

当初「高デューティが 1.28 秒で打ち切られるからでは」と書いたが、
**実測と合わない。**

- 境目は 10 秒で、桁が違う
- **T＝5 秒でもつながる。**そのときすでに低デューティなので、
  **低デューティでも繋がることが実測で示されている**

**原因は別のところにある。**

### 切り分け（未実施）

**失敗を作る**（右を入れ、15 秒待ってから左を入れる。1mA のまま）。
判定は右の消費電流。**RESET は 1 回だけ押す**（2 回はブートローダ）。

| # | 試すこと | 40µA に落ちたら | 1mA のままなら |
|---|---|---|---|
| **1** | **左だけリセット** | **左の走査の状態が原因** | 右側の問題 |
| 2 | 右だけリセット | ⚠️ **判断材料にならない**（下記） | **左が固まっている**（強い情報） |
| 3 | 両方に `CONFIG_ZMK_USB_LOGGING=y` を入れてログを読む | 状態遷移と失敗コードが直接読める | |

**1 が本命。**右を触らずに左だけ入れ直して繋がるなら、**右の広告は生きていて、
左が見つけられていなかった**ことになる。

> **⚠️ 2 で「直った」は、ほとんど何も証明しない。**
> 右をリセットすると高デューティの広告を最初からやり直す。そのとき左は
> すでに走査しているので、**繋がって当たり前**。
> 「右の状態が古かった」とも「新しい高デューティの窓に走査中の左が
> 居合わせた」とも読めてしまう。**直らなかったときだけ意味がある。**

**3 は焼き直しが要る**ので、1・2 のあと。

### 影響

**基板の発注はせき止めない**（ファームの問題で、基板は変わらない）。
**ただし出荷前に必ず閉じる。**「電源を入れたのに片方が効かない」は
故障として扱われる。

[#26](#26-片方が居ないともう片方が探し続けて電池を食う) と対になっている。
**両方が「相手が居ないと電池を食う」。**
