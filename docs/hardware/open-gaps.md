...
| # | 何 | 次の一手 |
|---|---|---|
| **#47** | **列の割り当てを直したが、本番の `pcb/` を作り直していない**（2026-08-17）。`hhkb_split.dtsi` の列割り当てを左（最下段 0/1/3）と右（行内の順番＝列番号・SW33/34 は例外で COL3/COL4）に直し、`pcb/matrix/` は作り直したが、**発注に使う `pcb/hhkb_split_{left,right}.kicad_pcb` は古い割り当てのまま**。そのため `test_matrix_wiring_matches_the_firmware` が左右とも赤（「SW25 が COL0 につながっていない」等）。**これは意図した状態**——利用者の指示で `pcb/matrix/` にだけ手を入れており、本番の配線（MCU 込み）は別途 | **マトリクスの形が固まってから** `gen_pcb → autoroute → drc` を回して `pcb/` を作り直す。そのとき `test_matrix_wiring_matches_the_firmware` と `test_the_routing_was_made_from_the_current_placement` が緑に戻るはず。**戻らなければ割り当ての写し取りが抜けている** |
