...
| **#51** | **プリンタが A1 mini に変わったら右の大物 3 点をどう刷るか**（2026-08-23・利用者「確定ではないがプリンタが A1 mini に変わる可能性が高い」）。case_right / topcase_right は幅 **179.7mm** でベッド 180mm に対し余裕 0.15mm/片側——スカートもブリムも引けず**実質刷れない**。plate_right（176.3mm）は手動配置なら限界圏。左と小物は全部入る。打ち手は **A: 右の大物だけ K1 Max／B: 2 分割して接合／C: 外注／D: 分割面の余白を 2.1mm 詰める（幅 177.6 に）**。**D の物理的裏付けは 2026-08-24 に実測済み**——試し板 177.6mm がベッド端で欠けなく刷れた。→ [a1-mini-feasibility.md](a1-mini-feasibility.md)（実スライスで検証済み。`slice_check.py --printer a1mini` で両にらみ可） | **プリンタが確定してから。**確定前に設計は変えない |
