...
**✅ 2026-08-25 決定: 作る**（利用者「Yes ただし SPI だけで十分？」→ 信号を再検討）。

| 板 | パッド（2.54 ピッチ・φ1.5・レジスト開口） | 位置（KiCad 座標） |
|---|---|---|
| 左 | TP1 SPI_SCK / TP2 SPI_MOSI / TP3 CS / TP4 V3V3 / TP5 GND | J_DB の奥・横一列 (200.0〜210.2, 71.6) |
| 右 | TP1 SPI_SCK / TP2 SPI_MOSI / TP3 CS / TP4 U1_U2 / TP5 V3V3 / TP6 GND | J_DB の左隣・縦一列 (66.9, 81.0〜93.7) |

ROW/COL はダイオードとソケットのパッドで見られるので出さない。子基板側はピンソケットで足りる。
V3V3 は電源の揺れ（ノイズ耐性の軸）を見るため、U1_U2 は右の 595 数珠つなぎの切り分け用。
実装: `circuit.py` の宣言（kind `testpoint`・実装なし＝`parts.NOT_ASSEMBLED`）、`gen_pcb.TP_PLACE` で
絶対座標に配置、回路図は「テストパッド」群。**残り: 各パッドから既存ネットへのスタブ（GND 以外）を
利用者が `matrix_only` で引く**（左 4 本・右 5 本）。引くまで DRC の未配線がその本数ぶん残る。
