...
> |---|---|---|
> | **普通のメスソケット 8.5mm** | **無加工。挿すだけ** | 8.5 ＋ 樹脂 2.0 ＝ **+10.5mm** |
> | 秋月 低メス 3.6mm | ピン 14 本を約 3mm に切り詰め | 3.6 ＋ 樹脂 2.0 ＝ **+5.6mm** |
>
> どちらも天井余裕 17mm の内側（+10.5mm でも天井まで約 7mm）。機能差は
> 無く、違いは USB 穴の上がる量とピン切りの手間だけ。**無加工で確実な
> 8.5mm を推奨**（ケースはどのみち再生成する）。
>
> なおヘッダ実装済みという事実は判断の重みも変える: **直付けには
> ヘッダ除去（14 ピンのはんだ吸い取り・パッド損傷リスク）が要り、
> ソケットなら現状のまま挿すだけ。**
- スタック増加を採ると `DB_STACK_H` 4.5 → **≈15.0mm**（普通のメス）／
  **≈10.1mm**（低メス＋ピン切り）。**買ってノギスで測ってから**入れる
  （provisional-values の作法）
