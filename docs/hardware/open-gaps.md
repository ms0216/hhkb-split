...
| [#41](#41-2026-08-14-電源まわりを作り直した残りは左の未配線-2-本) | **左の未配線 2 本**（ROW3/ROW4 が J_DB へ降りる枝）を塞ぐか | **利用者が「瑣末」と判断（2026-08-14）。**J_DB の ROW パッドの並びが行バスの y 順と食い違って交差する。XIAO のピン割り当てを変えれば解ける見込み。**自前配線では塞がない**（再現性が無い） |
| [#23](#23-未解決-アンテナが地板に挟まれている発注前に必ず読む) の前提 | **幾何の証明が無効になった。**「アンテナは必ず 4mm 以上 基板の下に入る」は成立しない（実測: 後端より **0.80mm 奥**） | #41 に実測を記録。**ただし余裕は 0.80mm しかない**ので、手 0（アルミ箔で RSSI・¥0・1 日）の価値は残る |
