...
| [#27](#27-ソケットは入る隙間-013mm-で入らないは保守平面の誤読だった2026-08-11) | ソケット（普通のメス 8.5mm・1×7 × 2） | **✅ 方針は 2026-08-12 に決定**（プロトタイプ期ソケット → 脱プロトタイプで直付け）。買って座高を測り `DB_STACK_H` 更新 → ケース再生成 |
...
### ソケットの購入候補（2026-08-11・Web 調査。座高は出所つき）
...
### ✅ 2026-08-12 決定（利用者）: プロトタイプ期はソケット、脱プロトタイプで直付けへ移行
