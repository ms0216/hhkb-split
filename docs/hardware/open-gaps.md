...
1. **ローカルの既定は「厳しい側」だけ実形状。**緩い側は記録の bbox 層に
   落とす（今日までのローカルの網羅は下げない）。「厳しい」は実測で決めた:
   **right** が全指標で上（立体 277 vs 240・全組 38,079 vs 28,549・
   bbox 近接 <0.1mm が 1,011 vs 871）。`gen_assembly.STRICT_HALF`
...
kicad-cli の無い環境は従来どおり記録の bbox＋予約の箱（その環境で作れる
いちばん正確な形）。checks ジョブのこの層は**モード B の重複ではない**
——安全余裕の箱 × ケースの組はこの層にしか出ない（下の「前提の穴」）。

### 先にやること（A/B どちらでも要る）

1. **GitHub Actions の二重化を解消する。**いま `checks`（モード A）と
   `real-shape`（モード B）が**両方とも全数の干渉確認をしている**。
   モード B は全部品を含むので、**CI での干渉確認はモード B 1 回でよい**。
   利用者の原則：「**全組み合わせを・重複なく 1 回・正確に**」
2. **`db` / `xiao` を実物の記録に置き換える。**基板まわりで
   **数値まで私が手で決めている**のはこの 2 つだけ（XIAO の実物モデルは
   すでにあり、品番も確定している）。`DB_STACK_H` だけは
   ...
   **測ってみたら、置き換え幅は当初の見立てより狭い**（2026-08-11）:
   ...
   - USB-C メスの位置・寸法は**すでに記録**（`usb_receptacle()`）から
     取っている
   - 残る置き換え候補は **xy の外形の出所**（`XIAO_OUTLINE_W` は
     Seeed の図面の手写し → 記録の bbox から取れる）だけ。
     ただし A（1 モード統一）なら `db`/`xiao` の箱ごと消えるので、
     **A/B を決めてから**やる
...
#29 実物を組み立て検査に入れる
  └→ 範囲が広がった: 全部品を実形状に ＋ 許容値ゼロ
       ...
       │    └→ db / xiao を実物の記録に置き換える → 置き換え幅は狭い（下記）。
       │        残りは xy 外形の出所だけ ← **いまここ（最後の枝）**
       └→ #30 スタビの切り欠きに逃げが無い（実形状の検査が見つけた本物）
```

**速度の話は寄り道ではなく、A を選べるようにするための前提**。
ここが片付いたら **A/B の判断 → #30 → 発注前の #23** に戻る。
...
### ★ メスがケースの外へ 0.26mm はみ出していた（**完成品の話。2026-08-11 に対策**）

**モデルの誤りではない。実物がそうなる。**利用者の判断:「はみ出しは避けたい。
外力がかかって故障の原因になる」。

**はみ出し量は `WALL + DB_FROM_REAR` だけで決まる**（コブを深くすると子基板も
一緒に奥へ動くので `BUMP_DEPTH` は効かない）。ここで**直し方に強い制約**がある:

> ⚠️ **子基板を前へ動かしてはいけない。**アンテナは XIAO の反対端にあり、
> いま本体基板の後端より **0.109mm** 外に出ているだけ（#23 を案 A で解いた
> ときの余裕）。前へ 0.3mm 動かせばアンテナは基板の下へ戻り、**#23 が
> 振り出しに戻る。**

なので `DB_FROM_REAR` を 0.4 → **0.955** に増やし、**同じだけコブを深くして**
子基板の絶対位置を保った（`BUMP_DEPTH` 18.0 → **18.555**）。

| | 前 | 後 |
|---|---|---|
| メスのはみ出し | **+0.255mm** | **−0.300mm**（引っ込む） |
| アンテナの余裕（#23） | 0.109mm | **0.109mm**（変わらない） |
| ケース奥行 | 125.12mm | **125.68mm**（コブが 0.56mm 深い。#2 に影響） |

**片方だけ動かすと落ちる**ように、2 つの検査が両側から挟んでいる:
`test_the_usb_receptacle_does_not_stick_out_of_the_case`（新設）と
`test_the_antenna_is_no_longer_under_the_main_board`。3 通りの変異で確認済み。

**この変更が #28 の検査を落とした。**壁が 0.555mm 厚くなったぶん、
**実測したケーブル（金属の露出 1.0mm）が 0.055mm 届かなくなった。**
座ぐりを 0.5 の直書きから `USB_PLUG_ENTRY − USB_SHELL_EXPOSED + CLEARANCE`
（＝0.755）へ、要求から導くように変えた。
...
| 物 | 理由 |
|---|---|
| 電池の電極バネ・リード線 | `batt` の占有空間に折込済み（`AA_TERMINAL` 8mm・幅 +1mm） |
...
| 物 | 数 | 気になる点 |
|---|---|---|
| キースイッチ本体 | 61 | **箱のまま。足さない理由を測って出した**（2026-08-10）。実物モデルの断面はプレートの中で 14.00 角・上で 15.60 角で、`SW_UNDER_W` / `SW_BODY_W` と**一致する**（箱は近似ではない）。実形状に替えて増えるのはステム・ピン・LED 窓だけで、どれも設計上ほかの部品の内側にあり、**嵌合なので必ず重なる**（例外が 2 つ増える）。総当たりは立体数の 2 乗で効くので 61×7 立体は実用外。→ `test_the_switch_boxes_match_the_real_switch` が**箱と実物のずれ**と**キャップの空洞にステムが収まるか**を見張る（3 定数で変異確認済み） |
...
| 電池のリード線・電極 | | 余裕は定数（AA_TERMINAL）で見ているだけ |
| M2 熱圧入インサート 14 本・ネジ | | |
