...
> **⚠️ 前提の更新（2026-08-11・利用者の申告）: 手持ちの XIAO には
> ブレッドボード用の角ピンヘッダが実装済み。**したがって**丸ピン候補は
> 不適**で、角ピン対応から選ぶ:
>
> | 道 | 座高 | 条件 |
> |---|---|---|
> | 秋月 低メス 2212S 系 | 3.6mm | 受け深さ 約 3.3mm。**ヘッダの下側ピン長を
> ノギスで測り**、長ければニッパで 3mm 級に切り詰める |
> | 普通のメスソケット | 8.5mm | 無加工で確実。天井余裕 17mm の内側だが、
> USB 穴が 8.5mm 上がる（ケース再生成時に干渉検査 0 を確認） |
>
> なおヘッダ実装済みという事実は判断の重みも変える: **直付けには
> ヘッダ除去（14 ピンのはんだ吸い取り・パッド損傷リスク）が要り、
> ソケットなら現状のまま挿すだけ。**
- スタック増加: 低メスなら `DB_STACK_H` 4.5 → **≈8.1mm**（座高 3.6 ＋
  樹脂 ≈2.5 が実測の対象）。普通のメスなら **≈13mm 級**。どちらも
  買って**ノギスで測ってから**入れる（provisional-values の作法）
