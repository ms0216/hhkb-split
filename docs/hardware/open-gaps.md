...
## 23. 子基板のアンテナ周りに金属が集中している

### 何が起きているか（実測）

子基板の配置（KiCad 座標。y が小さいほど奥＝USB 側）。
...
```
   ...
   92.4  XIAO の USB 端
  110.5  XIAO の**アンテナ端**
  111.0  J_MAIN（FFC コネクタ）★ アンテナから 0.5mm
  116.0  └─ 基板の縁
```

| | |
|---|---|
| GND ベタ | **アンテナ直下を覆い、5.2mm 先まで続く**（基板厚 1.6mm を隔てただけ） |
| 配線 | アンテナ近傍 6mm に **33 本**（B.Cu 24 / F.Cu 9） |
| FFC ケーブル | アンテナから 0.5mm のコネクタから、**12 芯が 50〜115mm 伸びる** |

**この案件は電池を 12.2mm 離すのに苦労し（#4）、`DB_ANTENNA_KEEPOUT = 10.0`
という定数まで作った。その定数は電池にしか適用されていない。**
`grep` すると参照は「電池との距離」の検査 1 箇所だけ。

### 構造的な制約

USB は「実機と同じく奥面へ出す」と決定済み（`decisions/2026-08-07-daughterboard.md`）。
そのため **XIAO の向きは固定**され、

- アンテナは USB の反対端＝**キーボード本体を向く側**
- FFC も本体基板へ繋ぐので**同じ側**

**アンテナと FFC は構造上どうしても同じ側になる。**FFC を奥側へ移すと、
今度はケーブルが基板を回り込んでアンテナの上を通る。

### なぜ「いま作り直さない」と決めたか

リンクバジェットを計算した（2.4GHz・送信 0dBm・受信感度 -90dBm）。
...
**必要な距離が非常に短く、余裕が大きい。**アンテナが 20〜30dB 劣化しても
30dB 残る。作り直しても「良くなったか」を測る手段が無いまま推測を重ねる
ことになる。

**子基板は 21×32mm で数百円。実物を作って測り、悪ければ改版する**ほうが
確実と判断した。

### 測る手順は用意した

[task-c3-ble-split.md](task-c3-ble-split.md) の **§6-3**。
**基準値はブレッドボードの段階（GND ベタもケーブルも無い＝最良の状態）で
取る。**子基板ができてからでは比較の原点が無い。

判定: 基準値から **20dB 以上悪化したら改版**（アンテナを基板の縁から
張り出させ、FFC を USB 側へ移す）。

## 24. M2 インサートの長さが無規定だった ✅ 2026-08-08 解決

`interface.M2_INSERT_D = 3.2mm`（外径）は規定されていたが、**長さは
どこにも書かれていなかった**。子基板の取付ボスは下穴を 5.0mm しか
掘っておらず（`gen_case.DB_BOSS_H + 1.0`）、**市販の 5.7mm を買うと
入らない**。

`envelopes.M2_INSERT_L`（暫定 4.0mm）を足し、
`test_the_m2_inserts_fit_the_bosses` が判定するようにした。
5.7mm を入れると落ちることを確認済み。

**外径だけ規定して長さを忘れる**のは、電池ボックスで「本数だけ指定して
配置を忘れた」のと同じ型。
