...
#29 実物を組み立て検査に入れる
  └→ 範囲が広がった: 全部品を実形状に ＋ 許容値ゼロ
       ...
       ├→ 全部品を実形状に → **未完**（この #31）
       │    ├→ A（1 モード）か B（2 モード）かを決める ← **いまここ（本丸）**
       │    ├→ A を選ぶとローカルが遅くなる → 速度の手当てが要る
       │    │    ├→ 時間の内訳を測った（下記）
       │    │    └→ ✅ 組分けを記録した: 片側 230 → 142 秒（2026-08-11）
       │    └→ db / xiao を実物の記録に置き換える
...
| 物 | 数 | 気になる点 |
|---|---|---|
| キースイッチ本体・キーキャップ | 61 | 高さは定数で効かせているが、**物としては置いていない** |
