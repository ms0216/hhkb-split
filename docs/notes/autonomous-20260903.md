...
| 9 | `tools/explain_overlap.py`（分割で幅が増える理由の説明図）が配列の写しを自前で持ち、最下段が 3u+3u のままだった。`layout/` の JSON（原機＋分割）から読むよう変更。右の島は数字段のキー数で原機の位置へ戻す | CLAUDE.md の作法 3「自分の生成物どうし」＋作法 8「置き換えたら古い方を消す」。`3u` の grep で見つけた唯一の取り残し | 図を出して目で見た（最下段が 2.75u×2＋空き）。数字は不変（7.25＋9.0＝16.25u・+23.8mm） | (下) | |
